...
static int fixture_setup(void** state)
{
    ...
    snprintf(fixture->fixture_directory, sizeof(fixture->fixture_directory), "/tmp/bc_integrity_verify_e2e_%d_XXXXXX", getpid());
...
static int run_bc_integrity_verify(const char* root, const char* manifest_path, const char* mode_argument)
{
    pid_t pid = fork();
    if (pid < 0) {
        return -1;
...
static int run_bc_integrity_verify_capture(const char* root, const char* manifest_path, const char* mode_argument,
                                           const char* format_argument, char* capture_output, size_t output_size)
{
    ...
    if (pid == 0) {
        ...
        close(pipefd[1]);
        char* argv[] = {
            (char*)BC_INTEGRITY_TEST_BINARY_PATH, "verify",    (char*)mode_argument, (char*)format_argument,
            "--default-exclude-virtual=false",    (char*)root, (char*)manifest_path, NULL,
        };
        execv(argv[0], argv);
        _exit(127);
    }
...
static void test_verify_identical_returns_zero(void** state)
{
    ...
    assert_int_equal(run_bc_integrity_manifest(fixture->fixture_directory, fixture->manifest_path), 0);
    assert_int_equal(run_bc_integrity_verify(fixture->fixture_directory, fixture->manifest_path, "--mode=strict"), 0);
}
...
static void test_verify_strict_detects_content_change(void** state)
{
    ...
    assert_int_equal(run_bc_integrity_verify(fixture->fixture_directory, fixture->manifest_path, "--mode=strict"), 1);
}
...
static void test_verify_content_only_ignores_meta_change(void** state)
{
    ...
    assert_int_equal(run_bc_integrity_verify(fixture->fixture_directory, fixture->manifest_path, "--mode=content"), 0);
}
...
static void test_verify_meta_only_detects_mode_change(void** state)
{
    ...
    snprintf(path, sizeof(path), "%s/sub/file2.txt", fixture->fixture_directory);
    assert_int_equal(chmod(path, 0700), 0);
    assert_int_equal(run_bc_integrity_verify(fixture->fixture_directory, fixture->manifest_path, "--mode=meta"), 1);
}
...
static void test_verify_json_emits_header_and_summary(void** state)
{
    ...
    assert_int_equal(run_bc_integrity_manifest(fixture->fixture_directory, fixture->manifest_path), 0);
    char output[16384];
    int exit_code = run_bc_integrity_verify_capture(fixture->fixture_directory, fixture->manifest_path, "--mode=strict", "--format=json",
...
static void test_verify_json_meta_change_includes_meta_changes(void** state)
{
    ...
    assert_int_equal(chmod(path, 0700), 0);
    char output[16384];
    int exit_code = run_bc_integrity_verify_capture(fixture->fixture_directory, fixture->manifest_path, "--mode=meta", "--format=json",
                                                    output, sizeof(output));
...
int main(void)
{
    const struct CMUnitTest tests[] = {
        cmocka_unit_test_setup_teardown(test_verify_identical_returns_zero, fixture_setup, fixture_teardown),
        cmocka_unit_test_setup_teardown(test_verify_strict_detects_content_change, fixture_setup, fixture_teardown),
        cmocka_unit_test_setup_teardown(test_verify_content_only_ignores_meta_change, fixture_setup, fixture_teardown),
        cmocka_unit_test_setup_teardown(test_verify_meta_only_detects_mode_change, fixture_setup, fixture_teardown),
        cmocka_unit_test_setup_teardown(test_verify_added_file_reported, fixture_setup, fixture_teardown),
        cmocka_unit_test_setup_teardown(test_verify_removed_file_reported, fixture_setup, fixture_teardown),
        cmocka_unit_test_setup_teardown(test_verify_json_emits_header_and_summary, fixture_setup, fixture_teardown),
        cmocka_unit_test_setup_teardown(test_verify_json_meta_change_includes_meta_changes, fixture_setup, fixture_teardown),
    };
