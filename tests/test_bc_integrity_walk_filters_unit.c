...
{
    (void)state;
    assert_true(bc_integrity_walk_is_hidden_segment(".bashrc", strlen(".bashrc")));
    assert_true(bc_integrity_walk_is_hidden_segment(".git", strlen(".git")));
    assert_true(bc_integrity_walk_is_hidden_segment(".cache/foo", strlen(".cache/foo")));
}

static void test_is_hidden_segment_inner_dot(void** state)
{
    (void)state;
    assert_true(bc_integrity_walk_is_hidden_segment("a/.git/b", strlen("a/.git/b")));
    assert_true(bc_integrity_walk_is_hidden_segment("foo/.cache", strlen("foo/.cache")));
    assert_true(bc_integrity_walk_is_hidden_segment("path/with/.dotted", strlen("path/with/.dotted")));
}

static void test_is_hidden_segment_negative(void** state)
{
    (void)state;
    assert_false(bc_integrity_walk_is_hidden_segment("regular.txt", strlen("regular.txt")));
...
static void test_is_virtual_root_negative(void** state)
{
    ...
    assert_false(bc_integrity_walk_is_virtual_root("/proc/version", strlen("/proc/version")));
    assert_false(bc_integrity_walk_is_virtual_root("/tmp/foo", strlen("/tmp/foo")));
}
...
static void test_is_virtual_subpath_filters_when_root_outside(void** state)
{
    ...
    assert_true(bc_integrity_walk_is_virtual_subpath(root, root_length, "/tmp/file", strlen("/tmp/file")));
    assert_false(bc_integrity_walk_is_virtual_subpath(root, root_length, "/etc/hosts", strlen("/etc/hosts")));
    assert_false(bc_integrity_walk_is_virtual_subpath(root, root_length, "/usr/bin/ls", strlen("/usr/bin/ls")));
}

static void test_is_virtual_subpath_not_filtered_when_root_inside(void** state)
{
    (void)state;
    const char* root = "/tmp/scan";
    size_t root_length = strlen("/tmp/scan");
    assert_false(bc_integrity_walk_is_virtual_subpath(root, root_length, "/tmp/scan/file", strlen("/tmp/scan/file")));
...
static void test_is_virtual_subpath_proc_in_etc_walk(void** state)
{
    ...
    assert_false(bc_integrity_walk_is_virtual_subpath(root, root_length, "/etc/hosts", strlen("/etc/hosts")));
    assert_false(bc_integrity_walk_is_virtual_subpath(root, root_length, "/etc/passwd", strlen("/etc/passwd")));
}
