...
static int fixture_setup(void** state)
{
    ...
    snprintf(fixture->fixture_directory, sizeof(fixture->fixture_directory), "/tmp/bc_integrity_hrbl_compat_%d_XXXXXX", getpid());
...
static int run_bc_hrbl_capture(const char* subcommand, const char* manifest_path, const char* path_argument, char* capture_output,
                               size_t output_size)
{
    int pipefd[2];
    if (pipe(pipefd) < 0) {
    ...
    if (pid == 0) {
        ...
        if (path_argument != NULL) {
            char* argv[] = {
                (char*)BC_HRBL_TEST_BINARY_PATH, (char*)subcommand, (char*)manifest_path, (char*)path_argument, NULL,
            };
            execv(argv[0], argv);
            _exit(127);
        }
...
static void test_hrbl_inspect_produces_valid_json(void** state)
{
    ...
    int inspect_exit = run_bc_hrbl_capture("inspect", fixture->manifest_path, NULL, inspect_output, sizeof(inspect_output));
    ...
    size_t lead = 0;
    while (lead < output_length &&
           (inspect_output[lead] == ' ' || inspect_output[lead] == '\t' || inspect_output[lead] == '\n' || inspect_output[lead] == '\r')) {
        ++lead;
    }
...
static void test_hrbl_query_finds_known_path(void** state)
{
    ...
    int query_exit = run_bc_hrbl_capture("query", fixture->manifest_path, "entries.'file_1.txt'.kind", query_output, sizeof(query_output));
    assert_int_equal(query_exit, 0);
    assert_non_null(strstr(query_output, "file"));

    char query_dir_output[256];
    int query_dir_exit =
        run_bc_hrbl_capture("query", fixture->manifest_path, "entries.sub.kind", query_dir_output, sizeof(query_dir_output));
    assert_int_equal(query_dir_exit, 0);
    assert_non_null(strstr(query_dir_output, "dir"));

    char query_lnk_output[256];
    int query_lnk_exit =
        run_bc_hrbl_capture("query", fixture->manifest_path, "entries.lnk.kind", query_lnk_output, sizeof(query_lnk_output));
...
int main(void)
{
    const struct CMUnitTest tests[] = {
        cmocka_unit_test_setup_teardown(test_hrbl_verify_accepts_bc_integrity_manifest, fixture_setup, fixture_teardown),
        cmocka_unit_test_setup_teardown(test_hrbl_inspect_produces_valid_json, fixture_setup, fixture_teardown),
        cmocka_unit_test_setup_teardown(test_hrbl_query_finds_known_path, fixture_setup, fixture_teardown),
    };
