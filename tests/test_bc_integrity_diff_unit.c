...
static int fixture_setup(void** state)
{
    ...
    snprintf(fixture->manifest_path_a, sizeof(fixture->manifest_path_a), "/tmp/bc_integrity_diff_unit_%d_a.hrbl", getpid());
    snprintf(fixture->manifest_path_b, sizeof(fixture->manifest_path_b), "/tmp/bc_integrity_diff_unit_%d_b.hrbl", getpid());
    *state = fixture;
    return 0;
}
...
static void make_file_entry(bc_integrity_entry_t* entry, const char* path, const char* digest_hex, uint64_t mode)
{
                           ...
                           size_t entries_count)
{
    ...
    for (size_t index = 0; index < entries_count; ++index) {
        assert_true(bc_containers_vector_push(memory_context, entries, &entries_array[index]));
    }
...
static void test_diff_added_removed_modified(void** state)
{
    ...
    make_file_entry(&entries_a[0], "alpha.txt", "1111111111111111111111111111111111111111111111111111111111111111", 0644);
    make_file_entry(&entries_a[1], "removed.txt", "3333333333333333333333333333333333333333333333333333333333333333", 0644);
    write_manifest(memory_context, fixture->manifest_path_a, entries_a, 2);

    bc_integrity_entry_t entries_b[2];
    make_file_entry(&entries_b[0], "added.txt", "5555555555555555555555555555555555555555555555555555555555555555", 0644);
    make_file_entry(&entries_b[1], "alpha.txt", "9999999999999999999999999999999999999999999999999999999999999999", 0644);
...
static void test_diff_identical_manifests_no_diff(void** state)
{
    ...
    make_file_entry(&entries[0], "file.txt", "1234567890123456789012345678901234567890123456789012345678901234", 0644);
...
int main(void)
{
    const struct CMUnitTest tests[] = {
        cmocka_unit_test_setup_teardown(test_diff_added_removed_modified, fixture_setup, fixture_teardown),
        cmocka_unit_test_setup_teardown(test_diff_identical_manifests_no_diff, fixture_setup, fixture_teardown),
    };
