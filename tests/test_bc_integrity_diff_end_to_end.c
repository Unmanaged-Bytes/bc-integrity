...
static int fixture_setup(void **state) {
  ...
  snprintf(fixture->fixture_directory, sizeof(fixture->fixture_directory),
           "/tmp/bc_integrity_diff_e2e_%d_XXXXXX", getpid());
...
static void test_diff_identical_manifests_returns_zero(void **state) {
  ...
  assert_int_equal(run_bc_integrity_diff(fixture->manifest_path_a,
                                         fixture->manifest_path_b, NULL, NULL,
                                         0),
                   0);
}
...
static void test_diff_added_removed_changed_reported(void **state) {
  ...
  snprintf(path, sizeof(path), "%s/will_remove.txt",
...
int main(void) {
  const struct CMUnitTest tests[] = {
      cmocka_unit_test_setup_teardown(
          test_diff_identical_manifests_returns_zero, fixture_setup,
          fixture_teardown),
      cmocka_unit_test_setup_teardown(test_diff_added_removed_changed_reported,
                                      fixture_setup, fixture_teardown),
      cmocka_unit_test_setup_teardown(test_diff_ignore_meta_skips_meta_changes,
                                      fixture_setup, fixture_teardown),
      cmocka_unit_test_setup_teardown(
          test_diff_json_emits_header_summary_and_meta_changes, fixture_setup,
          fixture_teardown),
  };
