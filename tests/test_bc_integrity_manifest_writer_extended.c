...
static int fixture_setup(void** state)
{
    ...
    snprintf(fixture->file_path, sizeof(fixture->file_path), "/tmp/bc_integrity_manifest_writer_ext_%d.hrbl", getpid());
    ...
    bc_containers_vector_create(fixture->memory_context, sizeof(bc_integrity_entry_t), 8, 4096, &fixture->entries);
    *state = fixture;
    return 0;
}
...
static void make_file_entry(bc_integrity_entry_t* entry, const char* path, const char* digest_hex)
{
...
static void make_options(bc_integrity_manifest_options_t* options, const char* output_path, bc_integrity_digest_algorithm_t algorithm)
{
...
static void test_writer_meta_block_complete_fields(void** state)
{
    ...
    make_file_entry(&entry, "s.txt", "1111111111111111111111111111111111111111111111111111111111111111");
    bc_containers_vector_push(fixture->memory_context, fixture->entries, &entry);

    bc_integrity_manifest_options_t options;
    make_options(&options, fixture->file_path, BC_INTEGRITY_DIGEST_ALGORITHM_SHA256);
    ...
    assert_true(bc_integrity_manifest_write_to_file(fixture->memory_context, &options, fixture->entries, &summary, fixture->file_path));

    bc_hrbl_reader_t* reader = NULL;
    assert_true(bc_hrbl_reader_open(fixture->memory_context, fixture->file_path, &reader));
    ...
    assert_true(bc_hrbl_reader_find(reader, "meta.root_path", strlen("meta.root_path"), &value_ref));
    assert_true(bc_hrbl_reader_get_string(&value_ref, &s_value, &s_length));
    ...
    assert_true(bc_hrbl_reader_find(reader, "meta.total_bytes", strlen("meta.total_bytes"), &value_ref));
    assert_true(bc_hrbl_reader_get_uint64(&value_ref, &u_value));
    assert_int_equal(u_value, 102400u);
    assert_true(bc_hrbl_reader_find(reader, "meta.walk_options.follow_symlinks", strlen("meta.walk_options.follow_symlinks"), &value_ref));
    assert_true(bc_hrbl_reader_find(reader, "meta.walk_options.include_hidden", strlen("meta.walk_options.include_hidden"), &value_ref));
...
static void test_writer_summary_block_fields(void** state)
{
    ...
    bc_hrbl_value_ref_t value_ref;
    uint64_t u_value = 0;
    assert_true(bc_hrbl_reader_find(reader, "summary.completed_at_unix_sec", strlen("summary.completed_at_unix_sec"), &value_ref));
    assert_true(bc_hrbl_reader_get_uint64(&value_ref, &u_value));
...
static void test_writer_empty_manifest_no_entries(void** state)
{
    ...
    bc_hrbl_value_ref_t value_ref;
    assert_true(bc_hrbl_reader_find(reader, "entries", strlen("entries"), &value_ref));
...
static void test_writer_error_entry_records_errno_and_message(void** state)
{
    ...
    bc_containers_vector_push(fixture->memory_context, fixture->entries, &error_entry);
    bc_integrity_manifest_options_t options;
    make_options(&options, fixture->file_path, BC_INTEGRITY_DIGEST_ALGORITHM_SHA256);
    bc_integrity_manifest_summary_t summary;
    make_summary(&summary);
    ...
    assert_true(bc_hrbl_reader_open(fixture->memory_context, fixture->file_path, &reader));
    bc_hrbl_value_ref_t value_ref;
    assert_true(bc_hrbl_reader_find(reader, "entries.'broken.txt'.errno", strlen("entries.'broken.txt'.errno"), &value_ref));
    int64_t i_value = 0;
    assert_true(bc_hrbl_reader_get_int64(&value_ref, &i_value));
    assert_int_equal(i_value, (int64_t)EACCES);
    assert_true(
        bc_hrbl_reader_find(reader, "entries.'broken.txt'.error_message", strlen("entries.'broken.txt'.error_message"), &value_ref));
    const char* msg_value = NULL;
    size_t msg_value_length = 0;
    assert_true(bc_hrbl_reader_get_string(&value_ref, &msg_value, &msg_value_length));
...
static void test_writer_skip_zero_length_relative_path(void** state)
{
    ...
    skipped.ok = true;
    bc_containers_vector_push(fixture->memory_context, fixture->entries, &skipped);
    bc_integrity_entry_t kept;
    make_file_entry(&kept, "kept.txt", "1111111111111111111111111111111111111111111111111111111111111111");
    ...
    assert_true(bc_hrbl_reader_open(fixture->memory_context, fixture->file_path, &reader));
    bc_hrbl_value_ref_t value_ref;
    assert_true(bc_hrbl_reader_find(reader, "entries.'kept.txt'.kind", strlen("entries.'kept.txt'.kind"), &value_ref));
    bc_hrbl_reader_close(reader);
}

static void check_algorithm_label(fixture_state_t* fixture, bc_integrity_digest_algorithm_t algorithm, const char* expected)
{
    bc_integrity_manifest_options_t options;
    ...
    const char* s_value = NULL;
    size_t s_length = 0;
    assert_true(bc_hrbl_reader_find(reader, "meta.digest_algorithm", strlen("meta.digest_algorithm"), &value_ref));
    assert_true(bc_hrbl_reader_get_string(&value_ref, &s_value, &s_length));
...
{
    check_algorithm_label((fixture_state_t*)*state, BC_INTEGRITY_DIGEST_ALGORITHM_XXH3, "xxh3");
}
static void test_writer_xxh128_label(void** state)
{
    check_algorithm_label((fixture_state_t*)*state, BC_INTEGRITY_DIGEST_ALGORITHM_XXH128, "xxh128");
}
...
static void test_writer_directory_entry_no_digest(void** state)
{
    ...
    bc_containers_vector_push(fixture->memory_context, fixture->entries, &dir_entry);
    bc_integrity_manifest_options_t options;
    make_options(&options, fixture->file_path, BC_INTEGRITY_DIGEST_ALGORITHM_SHA256);
    bc_integrity_manifest_summary_t summary;
    make_summary(&summary);
    ...
    const char* s_value = NULL;
    size_t s_length = 0;
    assert_true(bc_hrbl_reader_find(reader, "entries.subdir.kind", strlen("entries.subdir.kind"), &value_ref));
    assert_true(bc_hrbl_reader_get_string(&value_ref, &s_value, &s_length));
    assert_memory_equal(s_value, "dir", s_length);
    assert_false(bc_hrbl_reader_find(reader, "entries.subdir.digest_hex", strlen("entries.subdir.digest_hex"), &value_ref));
    bc_hrbl_reader_close(reader);
}
...
static void test_writer_stress_thousand_entries(void** state)
{
    ...
    for (size_t index = 0; index < entry_count; ++index) {
        ...
        make_file_entry(&entry_template, paths + index * 32, "1111111111111111111111111111111111111111111111111111111111111111");
        bc_containers_vector_push(fixture->memory_context, fixture->entries, &entry_template);
    }
    bc_integrity_manifest_options_t options;
    make_options(&options, fixture->file_path, BC_INTEGRITY_DIGEST_ALGORITHM_SHA256);
    bc_integrity_manifest_summary_t summary;
    ...
    bc_hrbl_value_ref_t value_ref;
    uint64_t u_value = 0;
    assert_true(bc_hrbl_reader_find(reader, "entries.'f_00000.dat'.kind", strlen("entries.'f_00000.dat'.kind"), &value_ref));
    assert_true(bc_hrbl_reader_find(reader, "entries.'f_01023.dat'.kind", strlen("entries.'f_01023.dat'.kind"), &value_ref));
    assert_true(bc_hrbl_reader_find(reader, "meta.file_count", strlen("meta.file_count"), &value_ref));
    assert_true(bc_hrbl_reader_get_uint64(&value_ref, &u_value));
...
static void test_writer_fifo_socket_device_kinds(void** state)
{
    ...
    for (size_t index = 0; index < 3; ++index) {
        bc_containers_vector_push(fixture->memory_context, fixture->entries, &entries[index]);
    }
    bc_integrity_manifest_options_t options;
    make_options(&options, fixture->file_path, BC_INTEGRITY_DIGEST_ALGORITHM_SHA256);
    bc_integrity_manifest_summary_t summary;
    ...
    const char* s_value = NULL;
    size_t s_length = 0;
    assert_true(bc_hrbl_reader_find(reader, "entries.fifo_node.kind", strlen("entries.fifo_node.kind"), &value_ref));
    assert_true(bc_hrbl_reader_get_string(&value_ref, &s_value, &s_length));
    assert_memory_equal(s_value, "fifo", s_length);
    assert_true(bc_hrbl_reader_find(reader, "entries.socket_node.kind", strlen("entries.socket_node.kind"), &value_ref));
    assert_true(bc_hrbl_reader_get_string(&value_ref, &s_value, &s_length));
    assert_memory_equal(s_value, "socket", s_length);
    assert_true(bc_hrbl_reader_find(reader, "entries.device_node.kind", strlen("entries.device_node.kind"), &value_ref));
    assert_true(bc_hrbl_reader_get_string(&value_ref, &s_value, &s_length));
...
int main(void)
{
    const struct CMUnitTest tests[] = {
        cmocka_unit_test_setup_teardown(test_writer_meta_block_complete_fields, fixture_setup, fixture_teardown),
        cmocka_unit_test_setup_teardown(test_writer_summary_block_fields, fixture_setup, fixture_teardown),
        cmocka_unit_test_setup_teardown(test_writer_empty_manifest_no_entries, fixture_setup, fixture_teardown),
        cmocka_unit_test_setup_teardown(test_writer_error_entry_records_errno_and_message, fixture_setup, fixture_teardown),
        cmocka_unit_test_setup_teardown(test_writer_skip_zero_length_relative_path, fixture_setup, fixture_teardown),
        cmocka_unit_test_setup_teardown(test_writer_xxh3_label, fixture_setup, fixture_teardown),
        cmocka_unit_test_setup_teardown(test_writer_xxh128_label, fixture_setup, fixture_teardown),
        cmocka_unit_test_setup_teardown(test_writer_directory_entry_no_digest, fixture_setup, fixture_teardown),
        cmocka_unit_test_setup_teardown(test_writer_stress_thousand_entries, fixture_setup, fixture_teardown),
        cmocka_unit_test_setup_teardown(test_writer_fifo_socket_device_kinds, fixture_setup, fixture_teardown),
    };
