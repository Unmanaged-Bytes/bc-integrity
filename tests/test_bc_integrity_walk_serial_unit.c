...
static int walk_serial_setup(void** state)
{
    ...
    snprintf(fixture->directory_path, sizeof(fixture->directory_path), "/tmp/bc_integrity_walk_serial_%d_XXXXXX", getpid());
...
static void test_walk_serial_completes_below_budget(void** state)
{
    ...
    bool walk_ok = bc_integrity_walk_run_serial_with_budget(fixture->memory_context, NULL, &options, fixture->directory_path,
...
static void test_walk_serial_bail_above_budget(void** state)
{
    ...
    bc_containers_vector_t* entries = NULL;
    assert_true(bc_containers_vector_create(fixture->memory_context, sizeof(bc_integrity_entry_t), 16, 1024, &entries));
...
static void test_walk_serial_zero_budget_unlimited(void** state)
{
    ...
    bool walk_ok = bc_integrity_walk_run_serial(fixture->memory_context, NULL, &options, fixture->directory_path, directory_path_length,
                                                entries, fixture->errors);
    assert_true(walk_ok);
...
int main(void)
{
    const struct CMUnitTest tests[] = {
        cmocka_unit_test_setup_teardown(test_walk_serial_completes_below_budget, walk_serial_setup, walk_serial_teardown),
        cmocka_unit_test_setup_teardown(test_walk_serial_bail_above_budget, walk_serial_setup, walk_serial_teardown),
        cmocka_unit_test_setup_teardown(test_walk_serial_zero_budget_unlimited, walk_serial_setup, walk_serial_teardown),
    };
