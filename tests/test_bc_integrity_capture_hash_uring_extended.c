...
static int fixture_setup(void **state) {
  ...
  snprintf(fixture->directory_path, sizeof(fixture->directory_path),
           "/tmp/bc_integrity_uring_ext_%d_XXXXXX", getpid());
...
static bc_integrity_hash_ring_t *alloc_ring(void) {
  bc_integrity_hash_ring_t *ring = (bc_integrity_hash_ring_t *)aligned_alloc(
      64, bc_integrity_hash_ring_struct_size());
...
static void test_consume_batch_small_files_sha256(void **state) {
  ...
  for (size_t index = 0; index < COUNT; ++index) {
    snprintf(paths[index], sizeof(paths[index]), "%s/file_%zu.dat",
             fixture->directory_path, index);
    write_file(paths[index], contents[index], strlen(contents[index]));
    bc_integrity_hash_consumer_begin(BC_INTEGRITY_DIGEST_ALGORITHM_SHA256,
                                     &states[index]);
  ...
  bc_integrity_hash_consumer_fn_t consumer =
      bc_integrity_hash_consumer_function_for(
  ...
  for (size_t index = 0; index < COUNT; ++index) {
    ...
    assert_true(bc_integrity_hash_finalize_into_hex(
        BC_INTEGRITY_DIGEST_ALGORITHM_SHA256, &states[index], actual_digest,
        &actual_length));
    assert_int_equal(actual_length, 64u);

    uint8_t reference[BC_CORE_SHA256_DIGEST_SIZE];
    assert_true(bc_core_sha256(contents[index], strlen(contents[index]),
                               reference));
...
static void test_consume_batch_xxh3_64(void **state) {
  ...
  bc_integrity_hash_consumer_begin(BC_INTEGRITY_DIGEST_ALGORITHM_XXH3,
                                   &consumer_state);
  ...
  assert_true(bc_integrity_hash_finalize_into_hex(
      BC_INTEGRITY_DIGEST_ALGORITHM_XXH3, &consumer_state, digest,
      &digest_length));
...
static void test_consume_batch_xxh128(void **state) {
  ...
  bc_integrity_hash_consumer_state_t consumer_state;
  bc_integrity_hash_consumer_begin(BC_INTEGRITY_DIGEST_ALGORITHM_XXH128,
                                   &consumer_state);
  bc_integrity_hash_batch_item_t item;
  item.absolute_path = path;
  item.file_size = strlen(content);
  ...
  size_t digest_length = 0;
  assert_true(bc_integrity_hash_finalize_into_hex(
      BC_INTEGRITY_DIGEST_ALGORITHM_XXH128, &consumer_state, digest,
      &digest_length));
...
static void test_consume_batch_more_than_slot_count(void **state) {
  ...
  bc_integrity_hash_consumer_state_t *states =
      malloc(sizeof(bc_integrity_hash_consumer_state_t) * total);
  bc_integrity_hash_batch_item_t *items =
      malloc(sizeof(bc_integrity_hash_batch_item_t) * total);
  assert_non_null(paths);
  assert_non_null(states);
  assert_non_null(items);

  for (size_t index = 0; index < total; ++index) {
    snprintf(paths[index], sizeof(paths[index]), "%s/big_%zu.dat",
             fixture->directory_path, index);
    char buffer[64];
...
static void test_consume_batch_empty_file_via_fallback(void **state) {
  ...
  size_t digest_length = 0;
  assert_true(bc_integrity_hash_finalize_into_hex(
      BC_INTEGRITY_DIGEST_ALGORITHM_SHA256, &consumer_state, digest,
      &digest_length));
  assert_string_equal(digest,
...
static void test_consume_batch_mixed_success_and_missing(void **state) {
  ...
  snprintf(path_missing, sizeof(path_missing), "%s/nope.txt",
...
static void test_consume_batch_at_slot_boundary(void **state) {
  ...
  bc_integrity_hash_consumer_state_t *states =
      malloc(sizeof(bc_integrity_hash_consumer_state_t) * count);
  bc_integrity_hash_batch_item_t *items =
      malloc(sizeof(bc_integrity_hash_batch_item_t) * count);
  assert_non_null(paths);
  assert_non_null(states);
  assert_non_null(items);
  for (size_t index = 0; index < count; ++index) {
    snprintf(paths[index], sizeof(paths[index]), "%s/b_%zu.dat",
             fixture->directory_path, index);
    char buffer[16];
...
static void test_consume_batch_many_missing_files(void **state) {
  ...
  bc_integrity_hash_batch_item_t items[COUNT];
  for (size_t index = 0; index < COUNT; ++index) {
    snprintf(paths[index], sizeof(paths[index]), "%s/missing_%zu.txt",
             fixture->directory_path, index);
    bc_integrity_hash_consumer_begin(BC_INTEGRITY_DIGEST_ALGORITHM_SHA256,
                                     &states[index]);
    items[index].absolute_path = paths[index];
...
int main(void) {
  const struct CMUnitTest tests[] = {
      ...
      cmocka_unit_test_setup_teardown(test_consume_batch_small_files_sha256,
                                      fixture_setup, fixture_teardown),
      cmocka_unit_test_setup_teardown(test_consume_batch_xxh3_64,
                                      fixture_setup, fixture_teardown),
      cmocka_unit_test_setup_teardown(test_consume_batch_xxh128, fixture_setup,
                                      fixture_teardown),
      cmocka_unit_test_setup_teardown(test_consume_batch_more_than_slot_count,
                                      fixture_setup, fixture_teardown),
      cmocka_unit_test_setup_teardown(test_consume_batch_empty_file_via_fallback,
                                      fixture_setup, fixture_teardown),
      cmocka_unit_test_setup_teardown(
          test_consume_batch_too_large_file_uses_fallback, fixture_setup,
          fixture_teardown),
      cmocka_unit_test_setup_teardown(test_consume_batch_mixed_success_and_missing,
                                      fixture_setup, fixture_teardown),
      cmocka_unit_test_setup_teardown(test_consume_batch_zero_items_succeeds,
                                      fixture_setup, fixture_teardown),
      cmocka_unit_test_setup_teardown(test_consume_batch_medium_file,
                                      fixture_setup, fixture_teardown),
      cmocka_unit_test_setup_teardown(test_consume_batch_at_slot_boundary,
                                      fixture_setup, fixture_teardown),
      cmocka_unit_test_setup_teardown(test_consume_batch_many_missing_files,
                                      fixture_setup, fixture_teardown),
  };
