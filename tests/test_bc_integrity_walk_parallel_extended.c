...
static int walk_parallel_setup(void **state) {
  ...
  snprintf(fixture->directory_path, sizeof(fixture->directory_path),
           "/tmp/bc_integrity_walk_par_%d_XXXXXX", getpid());
...
static void create_file_at(const char *parent, const char *name,
                           const char *contents) {
  char path[4096];
...
static void create_symlink_at(const char *parent, const char *target,
                              const char *name) {
  char path[4096];
  snprintf(path, sizeof(path), "%s/%s", parent, name);
...
static bc_containers_vector_t *make_entries_vector(walk_parallel_state_t *fix) {
  ...
  assert_true(bc_containers_vector_create(fix->memory_context,
                                          sizeof(bc_integrity_entry_t), 16,
                                          (size_t)1U << 24, &entries));
  return entries;
}

static void make_default_options(bc_integrity_manifest_options_t *options,
...
static void test_walk_parallel_small_tree(void **state) {
  ...
  bool walk_ok = bc_integrity_walk_run(
      fixture->memory_context, fixture->concurrency_context, NULL, &options,
...
static void test_walk_parallel_skip_special(void **state) {
  ...
  for (size_t index = 0; index < count; ++index) {
    ...
    if (entry.relative_path != NULL &&
        strcmp(entry.relative_path, "fifo_special") == 0) {
      found_fifo = true;
    }
  }
...
static void test_walk_parallel_with_symlinks_no_follow(void **state) {
  ...
  for (size_t index = 0; index < count; ++index) {
    ...
    assert_true(bc_containers_vector_get(entries, index, &entry));
    if (entry.relative_path != NULL &&
        strcmp(entry.relative_path, "link.txt") == 0) {
      found_symlink = true;
    }
  }
...
static void test_walk_parallel_default_exclude_virtual_root_rejected(
    void **state) {
  walk_parallel_state_t *fixture = (walk_parallel_state_t *)*state;
...
static void test_walk_parallel_filter_include_glob(void **state) {
  ...
  char child_dir[1024];
  snprintf(child_dir, sizeof(child_dir), "%s/child_dir",
  ...
  for (size_t index = 0; index < count; ++index) {
    ...
    const char *suffix =
        entry.relative_path + entry.relative_path_length - 4U;
...
static void test_walk_parallel_signal_stop_interrupts(void **state) {
  ...
  bool walk_ok = bc_integrity_walk_run(
      ...
      &options, fixture->directory_path, directory_path_length, entries,
      fixture->errors);
  (void)walk_ok;
...
static void test_walk_parallel_serial_when_one_worker(void **state) {
  ...
  assert_true(bc_concurrency_create(fixture->memory_context, &single_config,
                                    &single_worker_ctx));
  ...
  size_t directory_path_length = strlen(fixture->directory_path);
  bool walk_ok = bc_integrity_walk_run(
      fixture->memory_context, single_worker_ctx, NULL, &options,
      fixture->directory_path, directory_path_length, entries, fixture->errors);
  assert_true(walk_ok);
...
static void test_walk_parallel_unreadable_subdirectory(void **state) {
  ...
  snprintf(denied_path, sizeof(denied_path), "%s/denied",
           fixture->directory_path);
...
static void test_walk_parallel_default_exclude_virtual_non_virtual_root(
    void **state) {
  walk_parallel_state_t *fixture = (walk_parallel_state_t *)*state;
...
static void test_walk_parallel_subdirs_recursive(void **state) {
  ...
  int n1 = snprintf(level1_path, sizeof(level1_path), "%s/level1",
                    fixture->directory_path);
  ...
  int n3 = snprintf(deep_file_path, sizeof(deep_file_path), "%s/deep.txt",
                    level2_path);
  ...
  fputs("d", fp);
  fclose(fp);
  int n4 = snprintf(mid_file_path, sizeof(mid_file_path), "%s/mid.txt",
                    level1_path);
...
int main(void) {
  const struct CMUnitTest tests[] = {
      cmocka_unit_test_setup_teardown(test_walk_parallel_small_tree,
                                      walk_parallel_setup,
                                      walk_parallel_teardown),
      cmocka_unit_test_setup_teardown(
          test_walk_parallel_medium_tree_forces_parallel, walk_parallel_setup,
          walk_parallel_teardown),
      cmocka_unit_test_setup_teardown(test_walk_parallel_skip_hidden,
                                      walk_parallel_setup,
                                      walk_parallel_teardown),
      cmocka_unit_test_setup_teardown(test_walk_parallel_skip_special,
                                      walk_parallel_setup,
                                      walk_parallel_teardown),
      cmocka_unit_test_setup_teardown(test_walk_parallel_include_special,
                                      walk_parallel_setup,
                                      walk_parallel_teardown),
      cmocka_unit_test_setup_teardown(
          test_walk_parallel_with_symlinks_no_follow, walk_parallel_setup,
          walk_parallel_teardown),
      cmocka_unit_test_setup_teardown(test_walk_parallel_empty_tree,
                                      walk_parallel_setup,
                                      walk_parallel_teardown),
      cmocka_unit_test_setup_teardown(
          test_walk_parallel_default_exclude_virtual_root_rejected,
          walk_parallel_setup, walk_parallel_teardown),
      cmocka_unit_test_setup_teardown(test_walk_parallel_filter_include_glob,
                                      walk_parallel_setup,
                                      walk_parallel_teardown),
      cmocka_unit_test_setup_teardown(test_walk_parallel_filter_exclude_glob,
                                      walk_parallel_setup,
                                      walk_parallel_teardown),
      cmocka_unit_test_setup_teardown(
          test_walk_parallel_signal_stop_interrupts, walk_parallel_setup,
          walk_parallel_teardown),
      cmocka_unit_test_setup_teardown(test_walk_parallel_serial_when_one_worker,
                                      walk_parallel_setup,
                                      walk_parallel_teardown),
      cmocka_unit_test_setup_teardown(test_walk_parallel_subdirs_recursive,
                                      walk_parallel_setup,
                                      walk_parallel_teardown),
      cmocka_unit_test_setup_teardown(
          test_walk_parallel_default_exclude_virtual_non_virtual_root,
          walk_parallel_setup, walk_parallel_teardown),
      cmocka_unit_test_setup_teardown(test_walk_parallel_unreadable_subdirectory,
                                      walk_parallel_setup,
                                      walk_parallel_teardown),
  };
