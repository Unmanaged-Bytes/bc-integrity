...
#define BC_INTEGRITY_SMOKE_SCRIPT_PATH \
  "tests/smoke/smoke_golden_path.sh"
#endif
...
static int run_smoke_script(char *captured_output, size_t output_size,
  ...
  if (captured_output != NULL && output_size > 0) {
    while (total + 1 < output_size) {
      ssize_t bytes_read =
          read(pipefd[0], captured_output + total, output_size - 1 - total);
      if (bytes_read <= 0) {
        break;
      }
