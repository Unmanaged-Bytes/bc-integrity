...
static int fixture_setup(void** state)
{
    ...
    snprintf(fixture->fixture_directory, sizeof(fixture->fixture_directory), "/tmp/bc_integrity_repro_%d_XXXXXX", getpid());
...
static int fixture_teardown(void** state)
{
    ...
    char command[1024];
    snprintf(command, sizeof(command), "chmod -R u+rwx '%s' 2>/dev/null; rm -rf '%s'", fixture->fixture_directory,
...
static int run_bc_integrity_manifest(const char* root, const char* output, const char* threads_argument)
{
    pid_t pid = fork();
    if (pid < 0) {
        return -1;
...
static int run_bc_integrity_diff(const char* manifest_a, const char* manifest_b)
{
    pid_t pid = fork();
    if (pid < 0) {
        return -1;
...
static void load_entries(const char* manifest_path, entry_table_t* out_table)
{
    ...
    assert_true(bc_hrbl_reader_open(memory_context, manifest_path, &reader));

    bc_hrbl_value_ref_t entries_value;
    assert_true(bc_hrbl_reader_find(reader, "entries", strlen("entries"), &entries_value));
    ...
    while (bc_hrbl_iter_next(&iter, &child_value, &key, &key_length)) {
        ...
        size_t copy_length = key_length < sizeof(record->name) - 1 ? key_length : sizeof(record->name) - 1;
        ...
        while (bc_hrbl_iter_next(&field_iter, &field_value, &field_key, &field_key_length)) {
            if (field_key_length == strlen("digest_hex") && memcmp(field_key, "digest_hex", field_key_length) == 0) {
                const char* digest_data = NULL;
                size_t digest_length = 0;
                if (bc_hrbl_reader_get_string(&field_value, &digest_data, &digest_length)) {
                    size_t digest_copy = digest_length < sizeof(record->digest_hex) - 1 ? digest_length : sizeof(record->digest_hex) - 1;
                    memcpy(record->digest_hex, digest_data, digest_copy);
                    record->digest_hex[digest_copy] = '\0';
                    record->has_digest = true;
                }
            } else if (field_key_length == strlen("mode") && memcmp(field_key, "mode", field_key_length) == 0) {
                bc_hrbl_reader_get_uint64(&field_value, &record->mode);
            } else if (field_key_length == strlen("uid") && memcmp(field_key, "uid", field_key_length) == 0) {
                bc_hrbl_reader_get_uint64(&field_value, &record->uid);
            } else if (field_key_length == strlen("gid") && memcmp(field_key, "gid", field_key_length) == 0) {
                bc_hrbl_reader_get_uint64(&field_value, &record->gid);
            } else if (field_key_length == strlen("size_bytes") && memcmp(field_key, "size_bytes", field_key_length) == 0) {
                bc_hrbl_reader_get_uint64(&field_value, &record->size_bytes);
            } else if (field_key_length == strlen("ino") && memcmp(field_key, "ino", field_key_length) == 0) {
    ...
    qsort(out_table->records, out_table->count, sizeof(entry_record_t), compare_entry_record);

    bc_hrbl_reader_close(reader);
    bc_allocators_context_destroy(memory_context);
}

static void assert_tables_equivalent(const entry_table_t* table_a, const entry_table_t* table_b)
{
...
static void test_repeated_manifest_produces_identical_digests(void** state)
{
    ...
    assert_int_equal(run_bc_integrity_manifest(fixture->fixture_directory, fixture->manifest_path_a, NULL), 0);
    assert_int_equal(run_bc_integrity_manifest(fixture->fixture_directory, fixture->manifest_path_b, NULL), 0);
...
static void test_threads_modes_produce_identical_digests(void** state)
{
    ...
    assert_int_equal(run_bc_integrity_manifest(fixture->fixture_directory, fixture->manifest_path_a, "--threads=0"), 0);
    assert_int_equal(run_bc_integrity_manifest(fixture->fixture_directory, fixture->manifest_path_b, "--threads=auto"), 0);
...
static void test_diff_two_identical_manifests_returns_zero(void** state)
{
    ...
    int diff_exit = run_bc_integrity_diff(fixture->manifest_path_a, fixture->manifest_path_b);
...
int main(void)
{
    const struct CMUnitTest tests[] = {
        cmocka_unit_test_setup_teardown(test_repeated_manifest_produces_identical_digests, fixture_setup, fixture_teardown),
        cmocka_unit_test_setup_teardown(test_threads_modes_produce_identical_digests, fixture_setup, fixture_teardown),
        cmocka_unit_test_setup_teardown(test_diff_two_identical_manifests_returns_zero, fixture_setup, fixture_teardown),
    };
