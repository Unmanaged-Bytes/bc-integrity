...
{
    (void)state;
    assert_string_equal(bc_integrity_cli_digest_algorithm_name(BC_INTEGRITY_DIGEST_ALGORITHM_SHA256), "sha256");
    assert_string_equal(bc_integrity_cli_digest_algorithm_name(BC_INTEGRITY_DIGEST_ALGORITHM_XXH3), "xxh3");
    assert_string_equal(bc_integrity_cli_digest_algorithm_name(BC_INTEGRITY_DIGEST_ALGORITHM_XXH128), "xxh128");
}

int main(void)
{
    const struct CMUnitTest tests[] = {
        cmocka_unit_test(test_program_spec_pointer_is_non_null), cmocka_unit_test(test_program_spec_identity),
        cmocka_unit_test(test_program_spec_commands_declared),   cmocka_unit_test(test_program_spec_global_threads_option),
        cmocka_unit_test(test_program_spec_manifest_options),    cmocka_unit_test(test_digest_algorithm_name_round_trip),
    };
