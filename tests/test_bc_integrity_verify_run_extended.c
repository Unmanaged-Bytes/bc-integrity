...
static int fixture_setup(void** state)
{
    ...
    snprintf(fixture->fixture_directory, sizeof(fixture->fixture_directory), "/tmp/bc_integrity_verify_ext_%d_XXXXXX", getpid());
...
static int run_manifest(const char* root, const char* output, const char* algorithm)
{
    pid_t pid = fork();
    if (pid < 0) {
        return -1;
    ...
    if (pid == 0) {
        ...
        if (algorithm != NULL) {
            snprintf(algorithm_argument, sizeof(algorithm_argument), "--digest-algorithm=%s", algorithm);
        }
        char* argv[] = {
...
static int run_verify(const char* root, const char* manifest_path, const char* const* extra_args, size_t extra_args_count,
...
static void test_verify_corrupted_manifest_returns_two(void** state)
{
    ...
    assert_int_equal(run_verify(fixture->fixture_directory, fixture->manifest_path, args, 1, NULL, 0), 2);
}
...
static void test_verify_missing_manifest_returns_two(void** state)
{
    ...
    snprintf(missing_manifest, sizeof(missing_manifest), "%s/no_such.hrbl", fixture->fixture_directory);
    const char* args[] = {"--mode=strict"};
    assert_int_equal(run_verify(fixture->fixture_directory, missing_manifest, args, 1, NULL, 0), 2);
}
...
static void test_verify_root_inexistent_returns_two(void** state)
{
    fixture_state_t* fixture = (fixture_state_t*)*state;
    build_basic_tree(fixture->fixture_directory);
    assert_int_equal(run_manifest(fixture->fixture_directory, fixture->manifest_path, NULL), 0);
    char missing_root[400];
    snprintf(missing_root, sizeof(missing_root), "%s/inexistant_subdir", fixture->fixture_directory);
    const char* args[] = {"--mode=strict"};
    assert_int_equal(run_verify(missing_root, fixture->manifest_path, args, 1, NULL, 0), 2);
}
...
static void test_verify_root_is_file_returns_two(void** state)
{
    ...
    assert_int_equal(run_manifest(fixture->fixture_directory, fixture->manifest_path, NULL), 0);
    char file_root[400];
    snprintf(file_root, sizeof(file_root), "%s/alpha.txt", fixture->fixture_directory);
    const char* args[] = {"--mode=strict"};
    assert_int_equal(run_verify(file_root, fixture->manifest_path, args, 1, NULL, 0), 2);
}
...
static void test_verify_exit_on_first_change_short_circuits(void** state)
{
    ...
    assert_int_equal(run_verify(fixture->fixture_directory, fixture->manifest_path, args, 2, NULL, 0), 1);
}
...
static void test_verify_exit_on_first_with_removed_short_circuits(void** state)
{
    ...
    char path1[512];
    char path2[512];
    snprintf(path1, sizeof(path1), "%s/zzz_removed_one.txt", fixture->fixture_directory);
    write_file(path1, "first");
    snprintf(path2, sizeof(path2), "%s/zzz_removed_two.txt", fixture->fixture_directory);
...
static void test_verify_json_added_emits_change_record(void** state)
{
    ...
    char output[32768];
    int exit_code = run_verify(fixture->fixture_directory, fixture->manifest_path, args, 2, output, sizeof(output));
...
static void test_verify_json_removed_emits_change_record(void** state)
{
    ...
    build_basic_tree(fixture->fixture_directory);
    char path_zeta[512];
    snprintf(path_zeta, sizeof(path_zeta), "%s/zeta.txt", fixture->fixture_directory);
...
static void test_verify_strict_mtime_change_detected(void** state)
{
    ...
    const char* args[] = {"--mode=strict"};
    assert_int_equal(run_verify(fixture->fixture_directory, fixture->manifest_path, args, 1, NULL, 0), 1);
}
...
static void test_verify_empty_directory_zero_changes(void** state)
{
    ...
    assert_int_equal(run_manifest(fixture->fixture_directory, fixture->manifest_path, NULL), 0);
    const char* args[] = {"--mode=strict"};
    assert_int_equal(run_verify(fixture->fixture_directory, fixture->manifest_path, args, 1, NULL, 0), 0);
}
...
static void test_verify_with_symlink_kind(void** state)
{
    ...
    snprintf(target_path, sizeof(target_path), "%s/target.txt", fixture->fixture_directory);
    write_file(target_path, "target-content");
    char link_path[512];
    snprintf(link_path, sizeof(link_path), "%s/link", fixture->fixture_directory);
...
int main(void)
{
    const struct CMUnitTest tests[] = {
        cmocka_unit_test_setup_teardown(test_verify_corrupted_manifest_returns_two, fixture_setup, fixture_teardown),
        cmocka_unit_test_setup_teardown(test_verify_missing_manifest_returns_two, fixture_setup, fixture_teardown),
        cmocka_unit_test_setup_teardown(test_verify_root_inexistent_returns_two, fixture_setup, fixture_teardown),
        cmocka_unit_test_setup_teardown(test_verify_root_is_file_returns_two, fixture_setup, fixture_teardown),
        cmocka_unit_test_setup_teardown(test_verify_exit_on_first_change_short_circuits, fixture_setup, fixture_teardown),
        cmocka_unit_test_setup_teardown(test_verify_exit_on_first_with_removed_short_circuits, fixture_setup, fixture_teardown),
        cmocka_unit_test_setup_teardown(test_verify_json_added_emits_change_record, fixture_setup, fixture_teardown),
        cmocka_unit_test_setup_teardown(test_verify_json_removed_emits_change_record, fixture_setup, fixture_teardown),
        cmocka_unit_test_setup_teardown(test_verify_strict_mtime_change_detected, fixture_setup, fixture_teardown),
        cmocka_unit_test_setup_teardown(test_verify_empty_directory_zero_changes, fixture_setup, fixture_teardown),
        cmocka_unit_test_setup_teardown(test_verify_json_summary_files_total_present, fixture_setup, fixture_teardown),
        cmocka_unit_test_setup_teardown(test_verify_with_symlink_kind, fixture_setup, fixture_teardown),
    };
