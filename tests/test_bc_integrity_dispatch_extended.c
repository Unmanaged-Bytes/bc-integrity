...
static int dispatch_ext_make_state(void** state, size_t worker_count)
{
    ...
    snprintf(fixture->directory_path, sizeof(fixture->directory_path), "/tmp/bc_integrity_dispatch_ext_%d_XXXXXX", getpid());
...
static void make_entry_file(bc_integrity_entry_t* entry, const char* absolute_path, size_t file_size)
{
...
static void test_dispatch_skips_directory_kind(void** state)
{
    ...
    entry_dir.ok = true;
    assert_true(bc_containers_vector_push(fixture->memory_context, entries, &entry_dir));
    ...
    entry_link.ok = true;
    assert_true(bc_containers_vector_push(fixture->memory_context, entries, &entry_link));
...
static void test_dispatch_skips_not_ok_entry(void** state)
{
    ...
    assert_true(bc_containers_vector_push(fixture->memory_context, entries, &entry));
...
static void test_dispatch_skips_already_digested(void** state)
{
    ...
    char file_path[512];
    snprintf(file_path, sizeof(file_path), "%s/already.txt", fixture->directory_path);
...
static void test_dispatch_open_failure_records_error(void** state)
{
    ...
    snprintf(missing_path, sizeof(missing_path), "%s/does_not_exist.bin", fixture->directory_path);
...
static void test_dispatch_parallel_above_threshold_with_ring(void** state)
{
    ...
    bc_containers_vector_t* entries = NULL;
    assert_true(bc_containers_vector_create(fixture->memory_context, sizeof(bc_integrity_entry_t), 16, 64, &entries));
...
static void test_dispatch_signal_stop_before_run(void** state)
{
    ...
    bc_containers_vector_t* entries = NULL;
    assert_true(bc_containers_vector_create(fixture->memory_context, sizeof(bc_integrity_entry_t), 8, 32, &entries));
    char* paths = malloc(entry_count * 512);
    assert_non_null(paths);
    for (size_t i = 0; i < entry_count; ++i) {
        char* path = paths + i * 512;
    ...
    assert_true(bc_integrity_dispatch_compute_digests(fixture->memory_context, fixture->concurrency_context, signal_handler,
                                                      BC_INTEGRITY_DIGEST_ALGORITHM_SHA256, entries));
...
static void test_dispatch_sequential_mixed_kinds(void** state)
{
    ...
    snprintf(file_path_a, sizeof(file_path_a), "%s/seq_a.txt", fixture->directory_path);
    snprintf(file_path_b, sizeof(file_path_b), "%s/seq_b.txt", fixture->directory_path);
    ...
    assert_true(bc_containers_vector_push(fixture->memory_context, entries, &entry_broken));
    ...
    assert_true(bc_containers_vector_push(fixture->memory_context, entries, &entry_pre));
...
static void test_dispatch_sequential_zero_size(void** state)
{
    ...
    char file_path_b[512];
    snprintf(file_path_b, sizeof(file_path_b), "%s/data.bin", fixture->directory_path);
    ...
    assert_true(bc_containers_vector_push(fixture->memory_context, entries, &entry_data));
    ...
    assert_true(bc_containers_vector_get(entries, 0, &check));
    assert_int_equal(check.digest_hex_length, 64u);
    assert_string_equal(check.digest_hex, "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855");
...
static void test_dispatch_xxh3_algorithm(void** state)
{
    ...
    char file_path[512];
    snprintf(file_path, sizeof(file_path), "%s/x.txt", fixture->directory_path);
...
int main(void)
{
    const struct CMUnitTest tests[] = {
        cmocka_unit_test_setup_teardown(test_dispatch_zero_entries_noop, setup_workers_4, dispatch_ext_teardown),
        cmocka_unit_test_setup_teardown(test_dispatch_skips_directory_kind, setup_workers_4, dispatch_ext_teardown),
        cmocka_unit_test_setup_teardown(test_dispatch_skips_not_ok_entry, setup_workers_4, dispatch_ext_teardown),
        cmocka_unit_test_setup_teardown(test_dispatch_skips_already_digested, setup_workers_4, dispatch_ext_teardown),
        cmocka_unit_test_setup_teardown(test_dispatch_open_failure_records_error, setup_workers_mono, dispatch_ext_teardown),
        cmocka_unit_test_setup_teardown(test_dispatch_parallel_above_threshold_with_ring, setup_workers_4, dispatch_ext_teardown),
        cmocka_unit_test_setup_teardown(test_dispatch_parallel_oversize_files, setup_workers_4, dispatch_ext_teardown),
        cmocka_unit_test_setup_teardown(test_dispatch_parallel_mixed_sizes, setup_workers_4, dispatch_ext_teardown),
        cmocka_unit_test_setup_teardown(test_dispatch_signal_stop_before_run, setup_workers_mono, dispatch_ext_teardown),
        cmocka_unit_test_setup_teardown(test_dispatch_signal_stop_parallel_path, setup_workers_4, dispatch_ext_teardown),
        cmocka_unit_test_setup_teardown(test_dispatch_parallel_ring_with_missing_file, setup_workers_4, dispatch_ext_teardown),
        cmocka_unit_test_setup_teardown(test_dispatch_parallel_oversize_with_missing_file, setup_workers_4, dispatch_ext_teardown),
        cmocka_unit_test_setup_teardown(test_dispatch_sequential_mixed_kinds, setup_workers_mono, dispatch_ext_teardown),
        cmocka_unit_test_setup_teardown(test_dispatch_sequential_zero_size, setup_workers_mono, dispatch_ext_teardown),
        cmocka_unit_test_setup_teardown(test_dispatch_xxh3_algorithm, setup_workers_4, dispatch_ext_teardown),
    };
