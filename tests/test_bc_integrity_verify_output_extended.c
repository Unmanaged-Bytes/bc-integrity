...
static void test_emit_text_added_removed_content(void** state)
{
    ...
    for (size_t i = 0; i < sizeof(cases) / sizeof(cases[0]); ++i) {
        ...
        assert_true(bc_integrity_verify_emit_change_text(&writer, cases[i].kind, "new.txt", 7, NULL, NULL));
...
static void test_emit_text_meta_all_uint_fields(void** state)
{
    ...
    assert_true(bc_integrity_verify_emit_change_text(&writer, BC_INTEGRITY_VERIFY_CHANGE_META, "p.bin", 5, &expected, &actual));
...
static void test_emit_text_meta_link_targets(void** state)
{
    ...
    for (size_t i = 0; i < sizeof(cases) / sizeof(cases[0]); ++i) {
        ...
        assert_true(bc_integrity_verify_emit_change_text(&writer, BC_INTEGRITY_VERIFY_CHANGE_META, "lnk", 3, &expected, &actual));
        const char* data = NULL;
        size_t length = 0;
        assert_true(bc_core_writer_buffer_data(&writer, &data, &length));
...
static void test_emit_text_meta_no_diff(void** state)
{
    ...
    assert_true(bc_integrity_verify_emit_change_text(&writer, BC_INTEGRITY_VERIFY_CHANGE_META, "f.txt", 5, &expected, &actual));
...
{
    (void)state;
    bc_integrity_verify_change_kind_t kinds[] = {BC_INTEGRITY_VERIFY_CHANGE_META, BC_INTEGRITY_VERIFY_CHANGE_NONE};
    for (size_t i = 0; i < sizeof(kinds) / sizeof(kinds[0]); ++i) {
        char buffer[BUFFER_SIZE];
        bc_core_writer_t writer;
        init_writer(&writer, buffer, sizeof(buffer));
        assert_true(bc_integrity_verify_emit_change_text(&writer, kinds[i], "x", 1, NULL, NULL));
        (void)bc_core_writer_destroy(&writer);
    }
}
...
static void test_emit_text_both(void** state)
{
    ...
    assert_true(bc_integrity_verify_emit_change_text(&writer, BC_INTEGRITY_VERIFY_CHANGE_BOTH, "x.bin", 5, &expected, &actual));
    ...
    init_writer(&writer, buffer, sizeof(buffer));
    assert_true(bc_integrity_verify_emit_change_text(&writer, BC_INTEGRITY_VERIFY_CHANGE_BOTH, "x", 1, NULL, NULL));
    (void)bc_core_writer_destroy(&writer);
}
...
static void test_emit_json_simple_kinds(void** state)
{
    ...
    struct {
        ...
        {BC_INTEGRITY_VERIFY_CHANGE_ADDED, "\"change\":\"added\""},     {BC_INTEGRITY_VERIFY_CHANGE_REMOVED, "\"change\":\"removed\""},
        {BC_INTEGRITY_VERIFY_CHANGE_CONTENT, "\"change\":\"content\""}, {BC_INTEGRITY_VERIFY_CHANGE_META, "\"change\":\"meta\""},
        {BC_INTEGRITY_VERIFY_CHANGE_BOTH, "\"change\":\"both\""},       {BC_INTEGRITY_VERIFY_CHANGE_NONE, "\"change\":\"none\""},
    };
    for (size_t i = 0; i < sizeof(cases) / sizeof(cases[0]); ++i) {
        char buffer[BUFFER_SIZE];
        bc_core_writer_t writer;
        init_writer(&writer, buffer, sizeof(buffer));
        assert_true(bc_integrity_verify_emit_change_json(&writer, cases[i].kind, "p", 1, NULL, NULL));
        const char* data = NULL;
        size_t length = 0;
        assert_true(bc_core_writer_buffer_data(&writer, &data, &length));
...
static void test_emit_json_content_with_digest(void** state)
{
    ...
    assert_true(bc_integrity_verify_emit_change_json(&writer, BC_INTEGRITY_VERIFY_CHANGE_CONTENT, "x", 1, &expected, &actual));

    const char* data = NULL;
    size_t length = 0;
    assert_true(bc_core_writer_buffer_data(&writer, &data, &length));
    assert_non_null(strstr(data, "\"digest\":{\"old\":\"aaaaaa\",\"new\":\"bbbbbb\"}"));
    (void)bc_core_writer_destroy(&writer);
}
...
static void test_emit_json_meta_uint_fields(void** state)
{
    ...
    assert_true(bc_integrity_verify_emit_change_json(&writer, BC_INTEGRITY_VERIFY_CHANGE_META, "p", 1, &expected, &actual));
    ...
    static const char* fields[] = {
        "\"mode\":{\"old\":",       "\"uid\":{\"old\":",        "\"gid\":{\"old\":", "\"mtime_sec\":{\"old\":",
        "\"mtime_nsec\":{\"old\":", "\"size_bytes\":{\"old\":", "\"ino\":{\"old\":", "\"nlink\":{\"old\":",
    };
...
static void test_emit_json_meta_link_targets(void** state)
{
    ...
    for (size_t i = 0; i < sizeof(cases) / sizeof(cases[0]); ++i) {
        ...
        assert_true(bc_integrity_verify_emit_change_json(&writer, BC_INTEGRITY_VERIFY_CHANGE_META, "lnk", 3, &expected, &actual));
        const char* data = NULL;
        size_t length = 0;
        assert_true(bc_core_writer_buffer_data(&writer, &data, &length));
...
static void test_emit_json_both_with_digest_and_meta(void** state)
{
    ...
    assert_true(bc_integrity_verify_emit_change_json(&writer, BC_INTEGRITY_VERIFY_CHANGE_BOTH, "y", 1, &expected, &actual));
...
static void test_emit_json_path_with_special_chars(void** state)
{
    ...
    assert_true(bc_integrity_verify_emit_change_json(&writer, BC_INTEGRITY_VERIFY_CHANGE_ADDED, path, length, NULL, NULL));
...
static void test_emit_json_header(void** state)
{
    ...
    static const char* fields[] = {
        "\"command\":\"diff\"",
        "\"root_path\":\"/root\"",
        "\"manifest_path\":\"/m.hrbl\"",
        "\"manifest_path_a\":\"/a.hrbl\"",
        "\"manifest_path_b\":\"/b.hrbl\"",
        "\"mode\":\"strict\"",
        "\"algorithm\":\"sha256\"",
...
static void test_emit_json_summary(void** state)
{
    ...
    assert_true(bc_core_writer_buffer_data(&writer, &data, &length));
    static const char* fields[] = {
        "\"type\":\"summary\"", "\"files_total\":100", "\"changes_total\":7", "\"added\":1",        "\"removed\":2",
        "\"content\":3",        "\"meta\":4",          "\"both\":5",          "\"errors_count\":6", "\"wall_ms\":1234",
    };
    for (size_t i = 0; i < sizeof(fields) / sizeof(fields[0]); ++i) {
        assert_non_null(strstr(data, fields[i]));
    }
...
static void test_emit_writer_too_small(void** state)
{
    ...
    assert_false(bc_integrity_verify_emit_change_text(&writer, BC_INTEGRITY_VERIFY_CHANGE_ADDED, "long_name", 9, NULL, NULL));
    (void)bc_core_writer_destroy(&writer);

    init_writer(&writer, tiny, sizeof(tiny));
    assert_false(bc_integrity_verify_emit_change_json(&writer, BC_INTEGRITY_VERIFY_CHANGE_ADDED, "long_name", 9, NULL, NULL));
    (void)bc_core_writer_destroy(&writer);
...
static void make_full_change_snapshots(bc_integrity_meta_snapshot_t* expected, bc_integrity_meta_snapshot_t* actual)
{
    make_snapshot(expected);
    make_snapshot(actual);
    actual->mode = 0700;
...
static void run_text_with_buffer_size(size_t size, bc_integrity_verify_change_kind_t kind)
{
    ...
    (void)bc_integrity_verify_emit_change_text(&writer, kind, "p.bin", 5, &expected, &actual);
    (void)bc_core_writer_destroy(&writer);
    free(buffer);
}

static void run_text_simple_with_buffer_size(size_t size, bc_integrity_verify_change_kind_t kind)
{
    char* buffer = malloc(size);
    assert_non_null(buffer);
    bc_core_writer_t writer;
    assert_true(bc_core_writer_init_buffer_only(&writer, buffer, size));
    (void)bc_integrity_verify_emit_change_text(&writer, kind, "name.txt", 8, NULL, NULL);
    (void)bc_core_writer_destroy(&writer);
    free(buffer);
}

static void run_text_link_with_buffer_size(size_t size, size_t old_len, size_t new_len)
{
    char* buffer = malloc(size);
    assert_non_null(buffer);
    bc_core_writer_t writer;
    ...
    (void)bc_integrity_verify_emit_change_text(&writer, BC_INTEGRITY_VERIFY_CHANGE_META, "lnk", 3, &expected, &actual);
    (void)bc_core_writer_destroy(&writer);
    free(buffer);
}

static void run_json_with_buffer_size(size_t size, bc_integrity_verify_change_kind_t kind)
{
    char* buffer = malloc(size);
    assert_non_null(buffer);
    bc_core_writer_t writer;
    ...
    bc_integrity_meta_snapshot_t actual;
    make_full_change_snapshots(&expected, &actual);
    (void)bc_integrity_verify_emit_change_json(&writer, kind,
                                               "p\"\\\n\r\t\x01"
                                               "b",
                                               9, &expected, &actual);
    (void)bc_core_writer_destroy(&writer);
...
static void run_json_link_with_buffer_size(size_t size, size_t old_len, size_t new_len)
{
    char* buffer = malloc(size);
    assert_non_null(buffer);
    bc_core_writer_t writer;
    ...
    actual.link_target = "new";
    actual.link_target_length = new_len;
    (void)bc_integrity_verify_emit_change_json(&writer, BC_INTEGRITY_VERIFY_CHANGE_META, "lnk", 3, &expected, &actual);
    (void)bc_core_writer_destroy(&writer);
    free(buffer);
}
...
{
    (void)state;
    bc_integrity_verify_change_kind_t kinds[] = {BC_INTEGRITY_VERIFY_CHANGE_ADDED, BC_INTEGRITY_VERIFY_CHANGE_REMOVED,
...
static void test_buffer_sweep_json_content_with_digest(void** state)
{
    ...
    for (size_t size = 1; size <= 256; ++size) {
        ...
        bc_core_writer_t writer;
        assert_true(bc_core_writer_init_buffer_only(&writer, buffer, size));
        (void)bc_integrity_verify_emit_change_json(&writer, BC_INTEGRITY_VERIFY_CHANGE_CONTENT, "x", 1, &expected, &actual);
        (void)bc_core_writer_destroy(&writer);
        free(buffer);
    }
}
...
static void test_buffer_sweep_json_special_path(void** state)
{
    ...
    for (size_t size = 1; size <= 96; ++size) {
        ...
        assert_true(bc_core_writer_init_buffer_only(&writer, buffer, size));
        const char* path = "a\"b\\c\nd\re\tf\x01";
        (void)bc_integrity_verify_emit_change_json(&writer, BC_INTEGRITY_VERIFY_CHANGE_ADDED, path, 12, NULL, NULL);
        (void)bc_core_writer_destroy(&writer);
        free(buffer);
    }
}
