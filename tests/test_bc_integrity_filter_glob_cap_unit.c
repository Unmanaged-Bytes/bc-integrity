...
extern bool bc_integrity_filter_glob_matches(const char *pattern,
                                             const char *value,
                                             size_t value_length);
...
static void test_glob_rejects_pattern_with_too_many_double_star(void **state) {
  ...
  assert_false(
      bc_integrity_filter_glob_matches(pattern, value, strlen(value)));
}
...
static void test_glob_normal_pattern_works(void **state) {
  (void)state;
  assert_true(bc_integrity_filter_glob_matches("**/*.c", "src/sub/main.c",
                                               strlen("src/sub/main.c")));
  assert_true(bc_integrity_filter_glob_matches("build/**", "build/release/x",
                                               strlen("build/release/x")));
  assert_false(bc_integrity_filter_glob_matches(
      "**/*.c", "src/main.txt", strlen("src/main.txt")));
}
