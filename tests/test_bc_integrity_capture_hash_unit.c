...
static int fixture_setup(void **state) {
  ...
  snprintf(fixture->directory_path, sizeof(fixture->directory_path),
           "/tmp/bc_integrity_hash_test_%d_XXXXXX", getpid());
...
static void test_compute_sha256_hello(void **state) {
  ...
  fixture_state_t *fixture = (fixture_state_t *)*state;
  char file_path[512];
  snprintf(file_path, sizeof(file_path), "%s/hello.txt",
  ...
  int errno_value = 0;
  assert_true(bc_integrity_hash_compute_for_algorithm(
      file_path, 6, BC_INTEGRITY_DIGEST_ALGORITHM_SHA256, digest_buffer,
...
static void test_compute_xxh3(void **state) {
  ...
  int errno_value = 0;
  assert_true(bc_integrity_hash_compute_for_algorithm(
      file_path, 3, BC_INTEGRITY_DIGEST_ALGORITHM_XXH3, digest_buffer,
      &digest_length, &errno_value));
...
static void test_compute_xxh128(void **state) {
  fixture_state_t *fixture = (fixture_state_t *)*state;
  char file_path[512];
  snprintf(file_path, sizeof(file_path), "%s/x128.bin",
           fixture->directory_path);
  ...
  int errno_value = 0;
  assert_true(bc_integrity_hash_compute_for_algorithm(
      file_path, 16, BC_INTEGRITY_DIGEST_ALGORITHM_XXH128, digest_buffer,
      &digest_length, &errno_value));
...
static void test_consume_batch_two_files(void **state) {
  ...
  snprintf(file_path_a, sizeof(file_path_a), "%s/a.txt",
           fixture->directory_path);
  snprintf(file_path_b, sizeof(file_path_b), "%s/b.txt",
           fixture->directory_path);
  ...
  assert_true(bc_integrity_hash_finalize_into_hex(
      BC_INTEGRITY_DIGEST_ALGORITHM_SHA256, &state_a, digest_a,
      &digest_a_length));
  assert_true(bc_integrity_hash_finalize_into_hex(
      BC_INTEGRITY_DIGEST_ALGORITHM_SHA256, &state_b, digest_b,
      &digest_b_length));
...
static void test_consume_batch_missing_file(void **state) {
  ...
  snprintf(file_path_missing, sizeof(file_path_missing), "%s/does_not_exist",
           fixture->directory_path);
...
int main(void) {
  const struct CMUnitTest tests[] = {
      cmocka_unit_test_setup_teardown(test_compute_sha256_hello, fixture_setup,
                                      fixture_teardown),
      cmocka_unit_test_setup_teardown(test_compute_xxh3, fixture_setup,
                                      fixture_teardown),
      cmocka_unit_test_setup_teardown(test_compute_xxh128, fixture_setup,
                                      fixture_teardown),
      cmocka_unit_test_setup_teardown(test_consume_batch_two_files,
                                      fixture_setup, fixture_teardown),
      cmocka_unit_test_setup_teardown(test_consume_batch_missing_file,
                                      fixture_setup, fixture_teardown),
  };
