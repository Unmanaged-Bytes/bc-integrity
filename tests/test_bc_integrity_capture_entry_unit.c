...
static int fixture_setup(void **state) {
  ...
  snprintf(fixture->directory_path, sizeof(fixture->directory_path),
           "/tmp/bc_integrity_capture_test_%d_XXXXXX", getpid());
...
static void test_capture_regular_file_sha256(void **state) {
  ...
  char file_path[512];
  snprintf(file_path, sizeof(file_path), "%s/file.txt",
  ...
  int dir_fd =
      open(fixture->directory_path, O_RDONLY | O_DIRECTORY | O_CLOEXEC);
  assert_true(dir_fd >= 0);

  bc_integrity_entry_t entry;
  bc_integrity_capture_entry_from_stat(
      memory_context, &stat_buffer, BC_INTEGRITY_DIGEST_ALGORITHM_SHA256,
      dir_fd, "file.txt", file_path, strlen(file_path), "file.txt",
      strlen("file.txt"), false, &entry);
  close(dir_fd);
...
static void test_capture_symlink(void **state) {
  ...
  char file_path[512];
  char link_path[512];
  snprintf(file_path, sizeof(file_path), "%s/target.txt",
           fixture->directory_path);
  snprintf(link_path, sizeof(link_path), "%s/link.txt",
           fixture->directory_path);
  ...
  bc_integrity_capture_entry_from_stat(
      memory_context, &stat_buffer, BC_INTEGRITY_DIGEST_ALGORITHM_SHA256,
      dir_fd, "link.txt", link_path, strlen(link_path), "link.txt",
      strlen("link.txt"), false, &entry);
  close(dir_fd);
...
static void test_capture_directory_no_digest(void **state) {
  ...
  bc_integrity_capture_entry_from_stat(
      memory_context, &stat_buffer, BC_INTEGRITY_DIGEST_ALGORITHM_SHA256,
      dir_fd, "subdir", sub_path, strlen(sub_path), "subdir", strlen("subdir"),
      false, &entry);
  close(dir_fd);
...
int main(void) {
  const struct CMUnitTest tests[] = {
      cmocka_unit_test_setup_teardown(test_capture_regular_file_sha256,
                                      fixture_setup, fixture_teardown),
      cmocka_unit_test_setup_teardown(test_capture_symlink, fixture_setup,
                                      fixture_teardown),
      cmocka_unit_test_setup_teardown(test_capture_directory_no_digest,
                                      fixture_setup, fixture_teardown),
      cmocka_unit_test_setup_teardown(test_compute_digest_xxh3, fixture_setup,
                                      fixture_teardown),
  };
