...
static void test_glob_trailing_star_after_match(void **state) {
  (void)state;
  assert_true(
      bc_integrity_filter_glob_matches("abc*", "abc", strlen("abc")));
...
static void test_glob_unclosed_bracket_treated_literally(void **state) {
  ...
  assert_false(
      bc_integrity_filter_glob_matches("[abc", "abc", strlen("abc")));
  assert_false(
      bc_integrity_filter_glob_matches("[abc", "xabc", strlen("xabc")));
}

static void test_glob_charset_with_slash_rejected(void **state) {
  (void)state;
  assert_false(
      bc_integrity_filter_glob_matches("[a/]xyz", "/xyz", strlen("/xyz")));
}
...
static void test_glob_double_star_at_start_matches_anything(void **state) {
  ...
  assert_true(
      bc_integrity_filter_glob_matches("**", "deep/nested/path", strlen("deep/nested/path")));
}
...
static void test_glob_double_star_middle(void **state) {
  ...
  assert_true(
      bc_integrity_filter_glob_matches("a/**/b", "a/x/b", strlen("a/x/b")));
  assert_true(bc_integrity_filter_glob_matches("a/**/b", "a/x/y/z/b",
                                               strlen("a/x/y/z/b")));
  assert_false(
      bc_integrity_filter_glob_matches("a/**/b", "a/x/c", strlen("a/x/c")));
}

static void test_glob_pattern_done_value_not_done(void **state) {
  (void)state;
  assert_false(
      bc_integrity_filter_glob_matches("foo", "foo/bar", strlen("foo/bar")));
}

static void test_glob_pattern_not_done_value_done(void **state) {
  (void)state;
  assert_false(
      bc_integrity_filter_glob_matches("foo/bar", "foo", strlen("foo")));
}

static void test_glob_charset_range(void **state) {
  (void)state;
  assert_true(bc_integrity_filter_glob_matches("[a-c].txt", "b.txt",
                                               strlen("b.txt")));
  assert_false(bc_integrity_filter_glob_matches("[a-c].txt", "z.txt",
                                                strlen("z.txt")));
  assert_true(bc_integrity_filter_glob_matches("[A-Z][0-9]", "B5",
                                               strlen("B5")));
}

static void test_glob_question_mark_in_middle(void **state) {
  (void)state;
  assert_true(bc_integrity_filter_glob_matches("?ello", "hello",
                                               strlen("hello")));
  assert_true(
      bc_integrity_filter_glob_matches("?ello", "jello", strlen("jello")));
  assert_false(
      bc_integrity_filter_glob_matches("?ello", "ello", strlen("ello")));
}

static void test_glob_star_with_charset(void **state) {
  (void)state;
  assert_true(bc_integrity_filter_glob_matches("[abc]*", "abacus",
                                               strlen("abacus")));
  assert_true(bc_integrity_filter_glob_matches("[abc]*", "ball",
                                               strlen("ball")));
  assert_false(bc_integrity_filter_glob_matches("[abc]*", "zebra",
                                                strlen("zebra")));
}
...
static void test_filter_create_with_empty_strings(void **state) {
  ...
  assert_true(bc_integrity_filter_accepts_path(filter, "x.txt",
                                               strlen("x.txt")));
  assert_true(bc_integrity_filter_accepts_directory(filter, "x",
                                                    strlen("x")));
...
static void test_filter_null_filter_accepts(void **state) {
  (void)state;
  assert_true(
      bc_integrity_filter_accepts_path(NULL, "anything", strlen("anything")));
  assert_true(bc_integrity_filter_accepts_directory(NULL, "anything",
                                                    strlen("anything")));
}
...
static void test_filter_directory_excluded_by_exact_pattern(void **state) {
  ...
  bc_integrity_filter_t *filter = NULL;
  assert_true(
      bc_integrity_filter_create(memory_context, NULL, "node_modules", &filter));
  assert_false(bc_integrity_filter_accepts_directory(
      filter, "node_modules", strlen("node_modules")));
  assert_true(bc_integrity_filter_accepts_directory(filter, "src",
                                                    strlen("src")));
...
static void test_filter_directory_double_star_prefix_excludes_all(void **state) {
  ...
  assert_false(bc_integrity_filter_accepts_directory(filter, "anything",
                                                     strlen("anything")));
...
static void test_filter_directory_double_star_prefix_branch(void **state) {
  ...
  bc_integrity_filter_t *filter = NULL;
  assert_true(
      bc_integrity_filter_create(memory_context, NULL, "**/*.tmp", &filter));
  assert_false(bc_integrity_filter_accepts_directory(filter, "subdir",
                                                     strlen("subdir")));
...
static void test_filter_directory_slash_double_star_suffix(void **state) {
  ...
  bc_integrity_filter_t *filter = NULL;
  assert_true(bc_integrity_filter_create(memory_context, NULL, "build/**",
                                         &filter));
  assert_false(bc_integrity_filter_accepts_directory(filter, "build",
                                                     strlen("build")));
...
static void test_glob_long_subdir_pattern_value_done_early(void **state) {
  (void)state;
  assert_false(bc_integrity_filter_glob_matches(
      "a/b/c/d/e/f", "a/b/c", strlen("a/b/c")));
}
...
static void test_filter_multi_pattern_split(void **state) {
  ...
  assert_true(
      bc_integrity_filter_accepts_path(filter, "main.c", strlen("main.c")));
  assert_true(bc_integrity_filter_accepts_path(filter, "header.h",
                                               strlen("header.h")));
  assert_true(bc_integrity_filter_accepts_path(filter, "readme.txt",
                                               strlen("readme.txt")));
  assert_false(bc_integrity_filter_accepts_path(filter, "secret123",
                                                strlen("secret123")));
  assert_false(
      bc_integrity_filter_accepts_path(filter, "main.cpp", strlen("main.cpp")));
...
static void test_filter_single_pattern_no_separator(void **state) {
  ...
  bc_integrity_filter_t *filter = NULL;
  assert_true(
      bc_integrity_filter_create(memory_context, "*.md", NULL, &filter));
  assert_true(
      bc_integrity_filter_accepts_path(filter, "doc.md", strlen("doc.md")));
  assert_false(bc_integrity_filter_accepts_path(filter, "doc.html",
                                                strlen("doc.html")));
