...
static int smallcorpus_make_state(void** state, size_t worker_count)
{
    ...
    snprintf(fixture->directory_path, sizeof(fixture->directory_path), "/tmp/bc_integrity_dispatch_smallcorpus_%d_XXXXXX", getpid());
...
static char payload_for_index(size_t index)
{
    return 'a' + (char)index;
}

static void run_dispatch_and_verify(smallcorpus_state_t* fixture, size_t entry_count)
{
    ...
    for (size_t index = 0; index < entry_count; ++index) {
        ...
        assert_string_equal(entry_check.digest_hex, expected_digest_for_payload(index % 10));
    }
...
{
    if (*state == NULL || !sha256_is_available() || bc_integrity_test_under_valgrind()) {
        skip();
        return;
    }
    smallcorpus_state_t* fixture = (smallcorpus_state_t*)*state;
...
int main(void)
{
    const struct CMUnitTest tests[] = {
        cmocka_unit_test_setup_teardown(test_fallback_below_threshold_workers4, setup_workers_4, smallcorpus_teardown),
        cmocka_unit_test_setup_teardown(test_parallel_above_threshold_workers4, setup_workers_4, smallcorpus_teardown),
        cmocka_unit_test_setup_teardown(test_fallback_forced_single_worker, setup_workers_mono, smallcorpus_teardown),
    };
