...
static void test_meta_ignore_mtime_skips_mtime(void **state) {
  ...
  assert_int_equal(bc_integrity_verify_compare_meta(&expected, &actual, true),
                   BC_INTEGRITY_VERIFY_CHANGE_NONE);
  assert_int_equal(bc_integrity_verify_compare_meta(&expected, &actual, false),
                   BC_INTEGRITY_VERIFY_CHANGE_META);
}
...
int main(void) {
  const struct CMUnitTest tests[] = {
      cmocka_unit_test(test_meta_identical_returns_none),
      cmocka_unit_test(test_meta_mode_change_detected),
      cmocka_unit_test(test_meta_uid_change_detected),
      cmocka_unit_test(test_meta_inode_change_detected),
