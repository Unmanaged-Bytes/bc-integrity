...
static void test_strict_ignore_mtime_skips_mtime(void **state) {
  ...
  assert_int_equal(bc_integrity_verify_compare_strict(&expected, &actual, true),
                   BC_INTEGRITY_VERIFY_CHANGE_NONE);
  assert_int_equal(
      bc_integrity_verify_compare_strict(&expected, &actual, false),
      BC_INTEGRITY_VERIFY_CHANGE_META);
...
int main(void) {
  const struct CMUnitTest tests[] = {
      cmocka_unit_test(test_strict_identical_returns_none),
      cmocka_unit_test(test_strict_digest_mismatch_returns_content),
      cmocka_unit_test(test_strict_mode_change_returns_meta),
      cmocka_unit_test(test_strict_both_change_returns_both),
      cmocka_unit_test(test_strict_ignore_mtime_skips_mtime),
      cmocka_unit_test(test_strict_link_target_change_returns_meta),
  };
