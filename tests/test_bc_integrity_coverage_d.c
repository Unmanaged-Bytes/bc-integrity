...
static int coverage_d_setup(void** state)
{
    ...
    snprintf(fixture->fixture_directory, sizeof(fixture->fixture_directory), "/tmp/bc_integrity_cov_d_%d_XXXXXX", getpid());
    ...
    snprintf(fixture->manifest_path_b, sizeof(fixture->manifest_path_b), "%s.b.hrbl", fixture->fixture_directory);
...
static void cov_write_file_bytes(const char* path, const void* data, size_t length)
{
    FILE* file = fopen(path, "wb");
    assert_non_null(file);
    if (length > 0) {
...
static int cov_run_collect(char* const argv[], char* capture_output, size_t output_size, char* capture_stderr, size_t stderr_size)
{
    int outfd[2];
    int errfd[2];
    if (pipe(outfd) < 0) {
    ...
    if (capture_output != NULL && output_size > 0) {
        while (out_total + 1 < output_size) {
            ssize_t bytes_read = read(outfd[0], capture_output + out_total, output_size - 1 - out_total);
            if (bytes_read <= 0) {
                break;
            }
    ...
    if (capture_stderr != NULL && stderr_size > 0) {
        while (err_total + 1 < stderr_size) {
            ssize_t bytes_read = read(errfd[0], capture_stderr + err_total, stderr_size - 1 - err_total);
            if (bytes_read <= 0) {
                break;
            }
...
static int cov_run_manifest_args(const char* root, const char* output, const char* const* extra_args, size_t extra_args_count)
{
    char* argv[24];
    size_t cursor = 0;
...
static int cov_run_with_args(const char* const* args, size_t args_count, char* capture_output, size_t output_size, char* capture_stderr,
                             size_t stderr_size)
{
    char* argv[24];
    size_t cursor = 0;
    ...
    argv[cursor] = NULL;
    return cov_run_collect(argv, capture_output, output_size, capture_stderr, stderr_size);
}
...
static void test_main_help_per_subcommand(void** state)
{
    ...
    char output[8192];
    int exit_code = cov_run_with_args(args_manifest, 2, output, sizeof(output), NULL, 0);
    assert_int_equal(exit_code, 0);
...
static void test_main_manifest_threads_zero_singlethreaded(void** state)
{
    ...
    int rc = cov_run_manifest_args(fixture->fixture_directory, fixture->manifest_path, extra, 1);
...
static void test_main_manifest_digest_xxh3(void** state)
{
    ...
    char output_arg[512];
    snprintf(output_arg, sizeof(output_arg), "--output=%s", fixture->manifest_path);
...
static void test_main_manifest_include_hidden(void** state)
{
    ...
    snprintf(hidden_path, sizeof(hidden_path), "%s/.hidden_one.txt", fixture->fixture_directory);
...
static void test_main_manifest_follow_symlinks(void** state)
{
    ...
    char link_path[512];
    snprintf(target_path, sizeof(target_path), "%s/file_a.txt", fixture->fixture_directory);
    snprintf(link_path, sizeof(link_path), "%s/sym_link", fixture->fixture_directory);
...
static void test_main_manifest_root_inexistent_exits_one(void** state)
{
    ...
    snprintf(missing_root, sizeof(missing_root), "%s/no_such_dir", fixture->fixture_directory);
    char* argv[8];
    size_t cursor = 0;
    argv[cursor++] = (char*)BC_INTEGRITY_TEST_BINARY_PATH;
    argv[cursor++] = (char*)"manifest";
...
static void test_main_manifest_root_is_file_exits_one(void** state)
{
    coverage_d_fixture_t* fixture = (coverage_d_fixture_t*)*state;
    char file_path[512];
    snprintf(file_path, sizeof(file_path), "%s/just_a_file.txt", fixture->fixture_directory);
...
static void test_main_diff_one_arg_exits_two(void** state)
{
    coverage_d_fixture_t* fixture = (coverage_d_fixture_t*)*state;
    cov_build_basic_tree(fixture->fixture_directory);
    cov_run_manifest_args(fixture->fixture_directory, fixture->manifest_path, NULL, 0);
...
static void test_main_diff_two_identical_manifests_exits_zero(void** state)
{
    coverage_d_fixture_t* fixture = (coverage_d_fixture_t*)*state;
    cov_build_basic_tree(fixture->fixture_directory);
    assert_int_equal(cov_run_manifest_args(fixture->fixture_directory, fixture->manifest_path, NULL, 0), 0);
    assert_int_equal(cov_run_manifest_args(fixture->fixture_directory, fixture->manifest_path_b, NULL, 0), 0);
...
static void test_main_verify_threads_zero_singlethreaded(void** state)
{
    ...
    cov_build_basic_tree(fixture->fixture_directory);
    assert_int_equal(cov_run_manifest_args(fixture->fixture_directory, fixture->manifest_path, NULL, 0), 0);
    const char* args[] = {"--threads=0", "verify", "--default-exclude-virtual=false", fixture->fixture_directory, fixture->manifest_path};
...
static void test_main_verify_meta_mode_no_rehash(void** state)
{
    ...
    cov_build_basic_tree(fixture->fixture_directory);
    assert_int_equal(cov_run_manifest_args(fixture->fixture_directory, fixture->manifest_path, NULL, 0), 0);
    const char* args[] = {"verify", "--mode=meta", "--default-exclude-virtual=false", fixture->fixture_directory, fixture->manifest_path};
    int rc = cov_run_with_args(args, 5, NULL, 0, NULL, 0);
    assert_int_equal(rc, 0);
}
...
static void test_main_verify_content_mode_compares_only_digest(void** state)
{
    ...
    cov_build_basic_tree(fixture->fixture_directory);
    assert_int_equal(cov_run_manifest_args(fixture->fixture_directory, fixture->manifest_path, NULL, 0), 0);
    const char* args[] = {"verify", "--mode=content", "--default-exclude-virtual=false", fixture->fixture_directory,
...
static void cov_make_strict_baseline(bc_integrity_meta_snapshot_t* snapshot)
{
    ...
    snapshot->digest_hex = "1111111111111111111111111111111111111111111111111111111111111111";
...
static void test_strict_content_only_change(void** state)
{
    ...
    cov_make_strict_baseline(&expected);
    cov_make_strict_baseline(&actual);
    actual.digest_hex = "9999999999999999999999999999999999999999999999999999999999999999";
...
static void test_strict_both_meta_and_content_change(void** state)
{
    ...
    cov_make_strict_baseline(&actual);
    actual.mode = 0100700;
    actual.digest_hex = "8888888888888888888888888888888888888888888888888888888888888888";
...
static void cov_make_file_entry(bc_integrity_entry_t* entry, const char* path, bc_integrity_entry_kind_t kind, const char* digest_hex,
                                uint64_t mode)
{
...
static void cov_write_simple_manifest(bc_allocators_context_t* memory_context, const char* output_path, bc_integrity_entry_t* entries_array,
...
static void test_diff_kind_change_file_to_dir(void** state)
{
    ...
    cov_make_file_entry(&entry_file, "x", BC_INTEGRITY_ENTRY_KIND_FILE, "1111111111111111111111111111111111111111111111111111111111111111",
                        0644);
    cov_make_file_entry(&entry_dir, "x", BC_INTEGRITY_ENTRY_KIND_DIRECTORY, NULL, 0755);
    cov_write_simple_manifest(fixture->memory_context, fixture->manifest_path, &entry_file, 1);
    cov_write_simple_manifest(fixture->memory_context, fixture->manifest_path_b, &entry_dir, 1);
...
static void test_diff_ignore_mtime_only_mtime_diff_zero(void** state)
{
    ...
    cov_make_file_entry(&entry_a, "x", BC_INTEGRITY_ENTRY_KIND_FILE, "1111111111111111111111111111111111111111111111111111111111111111",
                        0644);
    cov_make_file_entry(&entry_b, "x", BC_INTEGRITY_ENTRY_KIND_FILE, "1111111111111111111111111111111111111111111111111111111111111111",
                        0644);
    entry_b.mtime_sec = 1900000000;
    cov_write_simple_manifest(fixture->memory_context, fixture->manifest_path, &entry_a, 1);
    cov_write_simple_manifest(fixture->memory_context, fixture->manifest_path_b, &entry_b, 1);
    bc_integrity_diff_options_t options;
    memset(&options, 0, sizeof(options));
    options.manifest_path_a = fixture->manifest_path;
    options.manifest_path_b = fixture->manifest_path_b;
...
static void test_diff_json_format_added_only(void** state)
{
    coverage_d_fixture_t* fixture = (coverage_d_fixture_t*)*state;
    bc_integrity_entry_t entry_b;
    cov_make_file_entry(&entry_b, "added.txt", BC_INTEGRITY_ENTRY_KIND_FILE,
                        "1111111111111111111111111111111111111111111111111111111111111111", 0644);
    cov_write_simple_manifest(fixture->memory_context, fixture->manifest_path, NULL, 0);
    cov_write_simple_manifest(fixture->memory_context, fixture->manifest_path_b, &entry_b, 1);
    bc_integrity_diff_options_t options;
    memset(&options, 0, sizeof(options));
    options.manifest_path_a = fixture->manifest_path;
...
static void test_writer_path_with_dot_prefix_uses_quoting(void** state)
{
    ...
    cov_make_file_entry(&entry, ".dotfile", BC_INTEGRITY_ENTRY_KIND_FILE,
                        "1111111111111111111111111111111111111111111111111111111111111111", 0644);
    cov_write_simple_manifest(fixture->memory_context, fixture->manifest_path, &entry, 1);
    bc_hrbl_reader_t* reader = NULL;
    assert_true(bc_hrbl_reader_open(fixture->memory_context, fixture->manifest_path, &reader));
    bc_hrbl_value_ref_t value_ref;
    assert_true(bc_hrbl_reader_find(reader, "entries.'.dotfile'.kind", strlen("entries.'.dotfile'.kind"), &value_ref));
...
static void test_writer_path_with_slash_uses_quoting(void** state)
{
    coverage_d_fixture_t* fixture = (coverage_d_fixture_t*)*state;
    bc_integrity_entry_t entry;
    cov_make_file_entry(&entry, "sub/nested.txt", BC_INTEGRITY_ENTRY_KIND_FILE,
                        "1111111111111111111111111111111111111111111111111111111111111111", 0644);
    cov_write_simple_manifest(fixture->memory_context, fixture->manifest_path, &entry, 1);
    bc_hrbl_reader_t* reader = NULL;
    assert_true(bc_hrbl_reader_open(fixture->memory_context, fixture->manifest_path, &reader));
    bc_hrbl_value_ref_t value_ref;
    assert_true(bc_hrbl_reader_find(reader, "entries.'sub/nested.txt'.kind", strlen("entries.'sub/nested.txt'.kind"), &value_ref));
    bc_hrbl_reader_close(reader);
}
...
static void test_writer_path_simple_no_quoting(void** state)
{
    coverage_d_fixture_t* fixture = (coverage_d_fixture_t*)*state;
    bc_integrity_entry_t entry;
    cov_make_file_entry(&entry, "simple", BC_INTEGRITY_ENTRY_KIND_FILE, "1111111111111111111111111111111111111111111111111111111111111111",
                        0644);
    cov_write_simple_manifest(fixture->memory_context, fixture->manifest_path, &entry, 1);
    bc_hrbl_reader_t* reader = NULL;
    assert_true(bc_hrbl_reader_open(fixture->memory_context, fixture->manifest_path, &reader));
    bc_hrbl_value_ref_t value_ref;
    assert_true(bc_hrbl_reader_find(reader, "entries.simple.kind", strlen("entries.simple.kind"), &value_ref));
    bc_hrbl_reader_close(reader);
}
...
static void test_writer_summary_with_walltime_set(void** state)
{
    ...
    assert_true(bc_hrbl_reader_open(fixture->memory_context, fixture->manifest_path, &reader));
    bc_hrbl_value_ref_t value_ref;
    assert_true(bc_hrbl_reader_find(reader, "summary.walltime_ms", strlen("summary.walltime_ms"), &value_ref));
    uint64_t walltime = 0;
...
static void test_writer_with_no_host_uses_unknown(void** state)
{
    ...
    bc_containers_vector_t* entries = NULL;
    bc_containers_vector_create(fixture->memory_context, sizeof(bc_integrity_entry_t), 8, 4096, &entries);
    assert_true(bc_integrity_manifest_write_to_file(fixture->memory_context, &options, entries, &summary, fixture->manifest_path));
    bc_hrbl_reader_t* reader = NULL;
    assert_true(bc_hrbl_reader_open(fixture->memory_context, fixture->manifest_path, &reader));
    bc_hrbl_value_ref_t value_ref;
    assert_true(bc_hrbl_reader_find(reader, "meta.host", strlen("meta.host"), &value_ref));
...
static void test_capture_empty_file(void** state)
{
    coverage_d_fixture_t* fixture = (coverage_d_fixture_t*)*state;
    char file_path[512];
    snprintf(file_path, sizeof(file_path), "%s/empty.bin", fixture->fixture_directory);
    cov_write_file_bytes(file_path, NULL, 0);
    struct stat stat_buffer;
    assert_int_equal(lstat(file_path, &stat_buffer), 0);
    int dir_fd = open(fixture->fixture_directory, O_RDONLY | O_DIRECTORY | O_CLOEXEC);
    assert_true(dir_fd >= 0);
    bc_integrity_entry_t entry;
    bc_integrity_capture_entry_from_stat(fixture->memory_context, &stat_buffer, BC_INTEGRITY_DIGEST_ALGORITHM_SHA256, dir_fd, "empty.bin",
                                         file_path, strlen(file_path), "empty.bin", strlen("empty.bin"), false, &entry);
    close(dir_fd);
...
static void test_capture_broken_symlink(void** state)
{
    coverage_d_fixture_t* fixture = (coverage_d_fixture_t*)*state;
    char link_path[512];
    snprintf(link_path, sizeof(link_path), "%s/broken_link", fixture->fixture_directory);
    ...
    assert_true(dir_fd >= 0);
    bc_integrity_entry_t entry;
    bc_integrity_capture_entry_from_stat(fixture->memory_context, &stat_buffer, BC_INTEGRITY_DIGEST_ALGORITHM_SHA256, dir_fd, "broken_link",
                                         link_path, strlen(link_path), "broken_link", strlen("broken_link"), false, &entry);
    close(dir_fd);
...
static void test_capture_compute_digest_unreadable_file(void** state)
{
    ...
    char file_path[512];
    snprintf(file_path, sizeof(file_path), "%s/unreadable.txt", fixture->fixture_directory);
    ...
    int errno_value = 0;
    bool ok = bc_integrity_capture_compute_digest(file_path, 9, BC_INTEGRITY_DIGEST_ALGORITHM_SHA256, digest_buffer, &digest_length,
                                                  &errno_value);
    (void)ok;
...
{
    (void)state;
    assert_string_equal(bc_integrity_entry_kind_name(BC_INTEGRITY_ENTRY_KIND_FILE), "file");
    assert_string_equal(bc_integrity_entry_kind_name(BC_INTEGRITY_ENTRY_KIND_DIRECTORY), "dir");
    assert_string_equal(bc_integrity_entry_kind_name(BC_INTEGRITY_ENTRY_KIND_SYMLINK), "symlink");
    assert_string_equal(bc_integrity_entry_kind_name(BC_INTEGRITY_ENTRY_KIND_FIFO), "fifo");
    assert_string_equal(bc_integrity_entry_kind_name(BC_INTEGRITY_ENTRY_KIND_SOCKET), "socket");
    assert_string_equal(bc_integrity_entry_kind_name(BC_INTEGRITY_ENTRY_KIND_DEVICE), "device");
}
...
static void test_capture_fifo_kind_via_stat(void** state)
{
    coverage_d_fixture_t* fixture = (coverage_d_fixture_t*)*state;
    char fifo_path[512];
    snprintf(fifo_path, sizeof(fifo_path), "%s/my_fifo_capture", fixture->fixture_directory);
    ...
    assert_true(dir_fd >= 0);
    bc_integrity_entry_t entry;
    bc_integrity_capture_entry_from_stat(fixture->memory_context, &stat_buffer, BC_INTEGRITY_DIGEST_ALGORITHM_SHA256, dir_fd,
                                         "my_fifo_capture", fifo_path, strlen(fifo_path), "my_fifo_capture", strlen("my_fifo_capture"),
                                         false, &entry);
    close(dir_fd);
...
static void test_capture_socket_kind_via_stat(void** state)
{
    ...
    snprintf(socket_path, sizeof(socket_path), "%s/my_socket", fixture->fixture_directory);
    ...
    assert_true(dir_fd >= 0);
    bc_integrity_entry_t entry;
    bc_integrity_capture_entry_from_stat(fixture->memory_context, &stat_buffer, BC_INTEGRITY_DIGEST_ALGORITHM_SHA256, dir_fd, "my_socket",
                                         socket_path, strlen(socket_path), "my_socket", strlen("my_socket"), false, &entry);
    close(dir_fd);
...
static void test_capture_long_link_target(void** state)
{
    coverage_d_fixture_t* fixture = (coverage_d_fixture_t*)*state;
    char link_path[512];
    snprintf(link_path, sizeof(link_path), "%s/long_link", fixture->fixture_directory);
    ...
    assert_true(dir_fd >= 0);
    bc_integrity_entry_t entry;
    bc_integrity_capture_entry_from_stat(fixture->memory_context, &stat_buffer, BC_INTEGRITY_DIGEST_ALGORITHM_SHA256, dir_fd, "long_link",
                                         link_path, strlen(link_path), "long_link", strlen("long_link"), false, &entry);
    close(dir_fd);
    assert_int_equal(entry.kind, BC_INTEGRITY_ENTRY_KIND_SYMLINK);
    unlink(link_path);
}
...
static void test_capture_compute_digest_missing_file(void** state)
{
    coverage_d_fixture_t* fixture = (coverage_d_fixture_t*)*state;
    char file_path[512];
    snprintf(file_path, sizeof(file_path), "%s/never_exists.txt", fixture->fixture_directory);
    char digest_buffer[BC_INTEGRITY_DIGEST_HEX_BUFFER_SIZE];
    size_t digest_length = 0;
    int errno_value = 0;
    assert_false(bc_integrity_capture_compute_digest(file_path, 0, BC_INTEGRITY_DIGEST_ALGORITHM_SHA256, digest_buffer, &digest_length,
                                                     &errno_value));
...
static void test_walk_serial_with_include_filter(void** state)
{
    ...
    assert_true(bc_runtime_error_collector_create(fixture->memory_context, &errors));
    bc_containers_vector_t* entries = NULL;
    assert_true(bc_containers_vector_create(fixture->memory_context, sizeof(bc_integrity_entry_t), 16, 256, &entries));
    bc_integrity_manifest_options_t options;
    ...
    bool walk_ok = bc_integrity_walk_run_serial(fixture->memory_context, NULL, &options, fixture->fixture_directory, directory_path_length,
                                                entries, errors);
    assert_true(walk_ok);
    bc_containers_vector_destroy(fixture->memory_context, entries);
...
static void test_walk_serial_with_include_special(void** state)
{
    ...
    cov_build_basic_tree(fixture->fixture_directory);
    char fifo_path[512];
    snprintf(fifo_path, sizeof(fifo_path), "%s/my_fifo", fixture->fixture_directory);
    if (mkfifo(fifo_path, 0644) != 0) {
        skip();
        return;
    }
...
static void test_walk_serial_with_hidden_dir_skipped(void** state)
{
    ...
    cov_build_basic_tree(fixture->fixture_directory);
    char hidden_dir[512];
    snprintf(hidden_dir, sizeof(hidden_dir), "%s/.hidden_dir", fixture->fixture_directory);
...
static void test_walk_serial_root_inexistent_returns_false(void** state)
{
    ...
    bool walk_ok =
        bc_integrity_walk_run_serial(fixture->memory_context, NULL, &options, missing_root, strlen(missing_root), entries, errors);
...
static void test_walk_serial_virtual_root_blocked(void** state)
{
    ...
    bool walk_ok = bc_integrity_walk_run_serial(fixture->memory_context, NULL, &options, root, strlen(root), entries, errors);
    assert_false(walk_ok);
    bc_containers_vector_destroy(fixture->memory_context, entries);
    bc_runtime_error_collector_destroy(fixture->memory_context, errors);
}
...
static void test_verify_meta_only_mtime_change(void** state)
{
    ...
    assert_int_equal(cov_run_manifest_args(fixture->fixture_directory, fixture->manifest_path, NULL, 0), 0);
    char file_a[512];
    snprintf(file_a, sizeof(file_a), "%s/file_a.txt", fixture->fixture_directory);
...
static void test_verify_zero_changes_json(void** state)
{
    ...
    assert_int_equal(cov_run_manifest_args(fixture->fixture_directory, fixture->manifest_path, NULL, 0), 0);
    const char* args[] = {
        "verify", "--mode=strict", "--format=json", "--default-exclude-virtual=false", fixture->fixture_directory, fixture->manifest_path};
    char output[16384];
...
static void test_verify_many_entries_consistent(void** state)
{
    ...
    for (size_t index = 0; index < 25; ++index) {
        char path[600];
        snprintf(path, sizeof(path), "%s/file_%02zu.txt", fixture->fixture_directory, index);
        char content[64];
        snprintf(content, sizeof(content), "content-%zu", index);
        cov_write_file(path, content);
    }
    assert_int_equal(cov_run_manifest_args(fixture->fixture_directory, fixture->manifest_path, NULL, 0), 0);
    const char* args[] = {"verify", "--mode=strict", "--default-exclude-virtual=false", fixture->fixture_directory, fixture->manifest_path};
    int rc = cov_run_with_args(args, 5, NULL, 0, NULL, 0);
    assert_int_equal(rc, 0);
}
...
static void test_verify_socket_entry_in_manifest(void** state)
{
    ...
    cov_build_basic_tree(fixture->fixture_directory);
    char socket_path[512];
    snprintf(socket_path, sizeof(socket_path), "%s/mysock", fixture->fixture_directory);
    int sfd = socket(AF_UNIX, SOCK_STREAM, 0);
    if (sfd < 0) {
        skip();
        return;
...
static void test_verify_fifo_entry_in_manifest(void** state)
{
    ...
    cov_build_basic_tree(fixture->fixture_directory);
    char fifo_path[512];
    snprintf(fifo_path, sizeof(fifo_path), "%s/myfifo", fixture->fixture_directory);
    if (mkfifo(fifo_path, 0644) != 0) {
        skip();
        return;
    }
...
static void test_verify_entry_removed_from_filesystem(void** state)
{
    ...
    cov_build_basic_tree(fixture->fixture_directory);
    char extra[512];
    snprintf(extra, sizeof(extra), "%s/extra_to_remove.txt", fixture->fixture_directory);
...
static void test_cli_bind_manifest_via_full_parse_succeeds(void** state)
{
    ...
    const char* argv[] = {"bc-integrity", "manifest", "--output=/tmp/dummy.hrbl", "/tmp"};
    bc_runtime_cli_parsed_t parsed;
    bc_runtime_cli_parse_status_t status = bc_runtime_cli_parse(spec, 4, argv, store, &parsed, stderr);
...
static void test_cli_bind_verify_via_full_parse_succeeds(void** state)
{
    ...
    assert_true(bc_runtime_config_store_create(fixture->memory_context, &store));
    const bc_runtime_cli_program_spec_t* spec = bc_integrity_cli_program_spec();
    const char* argv[] = {"bc-integrity", "verify", "--mode=strict", "/tmp", "/tmp/m.hrbl"};
    bc_runtime_cli_parsed_t parsed;
    bc_runtime_cli_parse_status_t status = bc_runtime_cli_parse(spec, 5, argv, store, &parsed, stderr);
    assert_int_equal(status, BC_RUNTIME_CLI_PARSE_OK);
    bc_integrity_verify_options_t options;
...
static void test_cli_bind_diff_via_full_parse_succeeds(void** state)
{
    ...
    assert_true(bc_runtime_config_store_create(fixture->memory_context, &store));
    const bc_runtime_cli_program_spec_t* spec = bc_integrity_cli_program_spec();
    const char* argv[] = {"bc-integrity", "diff", "--format=json", "/tmp/a.hrbl", "/tmp/b.hrbl"};
    bc_runtime_cli_parsed_t parsed;
    bc_runtime_cli_parse_status_t status = bc_runtime_cli_parse(spec, 5, argv, store, &parsed, stderr);
    assert_int_equal(status, BC_RUNTIME_CLI_PARSE_OK);
    bc_integrity_diff_options_t options;
...
static void test_cli_bind_manifest_invalid_threads_emits_invalid(void** state)
{
    ...
    assert_true(bc_runtime_config_store_create(fixture->memory_context, &store));
    const bc_runtime_cli_program_spec_t* spec = bc_integrity_cli_program_spec();
    const char* argv[] = {"bc-integrity", "--threads=invalid_word", "manifest", "--output=/tmp/dummy.hrbl", "/tmp"};
    bc_runtime_cli_parsed_t parsed;
    bc_runtime_cli_parse_status_t status = bc_runtime_cli_parse(spec, 5, argv, store, &parsed, stderr);
    assert_int_equal(status, BC_RUNTIME_CLI_PARSE_OK);
    bc_integrity_manifest_options_t options;
...
static void test_cli_bind_verify_invalid_threads_emits_invalid(void** state)
{
    ...
    assert_true(bc_runtime_config_store_create(fixture->memory_context, &store));
    const bc_runtime_cli_program_spec_t* spec = bc_integrity_cli_program_spec();
    const char* argv[] = {"bc-integrity", "--threads=garbage", "verify", "/tmp", "/tmp/m.hrbl"};
    bc_runtime_cli_parsed_t parsed;
    bc_runtime_cli_parse_status_t status = bc_runtime_cli_parse(spec, 5, argv, store, &parsed, stderr);
    assert_int_equal(status, BC_RUNTIME_CLI_PARSE_OK);
    bc_integrity_verify_options_t options;
...
static void test_cli_bind_manifest_with_include_exclude_lists(void** state)
{
    ...
    assert_true(bc_runtime_config_store_create(fixture->memory_context, &store));
    const bc_runtime_cli_program_spec_t* spec = bc_integrity_cli_program_spec();
    const char* argv[] = {"bc-integrity",       "manifest",         "--output=/tmp/dummy.hrbl", "--include=*.txt",
                          "--exclude=junk.bin", "--include-hidden", "--follow-symlinks",        "/tmp"};
    bc_runtime_cli_parsed_t parsed;
    bc_runtime_cli_parse_status_t status = bc_runtime_cli_parse(spec, 8, argv, store, &parsed, stderr);
    assert_int_equal(status, BC_RUNTIME_CLI_PARSE_OK);
    bc_integrity_manifest_options_t options;
    assert_true(bc_integrity_cli_bind_manifest_options(store, &parsed, &options));
...
static void test_cli_bind_verify_with_include_exclude_lists(void** state)
{
    ...
    assert_true(bc_runtime_config_store_create(fixture->memory_context, &store));
    const bc_runtime_cli_program_spec_t* spec = bc_integrity_cli_program_spec();
    const char* argv[] = {"bc-integrity",       "verify",          "--mode=meta", "--format=json", "--include=*.txt",
                          "--exclude=junk.bin", "--exit-on-first", "/tmp",        "/tmp/m.hrbl"};
    bc_runtime_cli_parsed_t parsed;
    bc_runtime_cli_parse_status_t status = bc_runtime_cli_parse(spec, 9, argv, store, &parsed, stderr);
    assert_int_equal(status, BC_RUNTIME_CLI_PARSE_OK);
    bc_integrity_verify_options_t options;
    assert_true(bc_integrity_cli_bind_verify_options(store, &parsed, &options));
...
static void test_cli_bind_diff_with_ignore_flags(void** state)
{
    ...
    assert_true(bc_runtime_config_store_create(fixture->memory_context, &store));
    const bc_runtime_cli_program_spec_t* spec = bc_integrity_cli_program_spec();
    const char* argv[] = {"bc-integrity", "diff", "--format=text", "--ignore-meta", "--ignore-mtime", "/tmp/a.hrbl", "/tmp/b.hrbl"};
    bc_runtime_cli_parsed_t parsed;
    bc_runtime_cli_parse_status_t status = bc_runtime_cli_parse(spec, 7, argv, store, &parsed, stderr);
    assert_int_equal(status, BC_RUNTIME_CLI_PARSE_OK);
    bc_integrity_diff_options_t options;
    assert_true(bc_integrity_cli_bind_diff_options(store, &parsed, &options));
...
static void test_main_diff_format_json_with_changes(void** state)
{
    ...
    assert_int_equal(cov_run_manifest_args(fixture->fixture_directory, fixture->manifest_path_b, NULL, 0), 0);
    const char* args[] = {"diff", "--format=json", fixture->manifest_path, fixture->manifest_path_b};
    char output[16384];
...
static void test_main_diff_ignore_meta_zero(void** state)
{
    ...
    chmod(path, 0600);
    assert_int_equal(cov_run_manifest_args(fixture->fixture_directory, fixture->manifest_path_b, NULL, 0), 0);
    const char* args[] = {"diff", "--ignore-meta", fixture->manifest_path, fixture->manifest_path_b};
...
static void test_main_diff_with_text_format_explicit(void** state)
{
    ...
    assert_int_equal(cov_run_manifest_args(fixture->fixture_directory, fixture->manifest_path, NULL, 0), 0);
    assert_int_equal(cov_run_manifest_args(fixture->fixture_directory, fixture->manifest_path_b, NULL, 0), 0);
    const char* args[] = {"diff", "--format=text", fixture->manifest_path, fixture->manifest_path_b};
    int rc = cov_run_with_args(args, 4, NULL, 0, NULL, 0);
    assert_int_equal(rc, 0);
}
...
static void test_main_verify_invalid_mode_exits_two(void** state)
{
    ...
    cov_build_basic_tree(fixture->fixture_directory);
    assert_int_equal(cov_run_manifest_args(fixture->fixture_directory, fixture->manifest_path, NULL, 0), 0);
    const char* args[] = {"verify", "--mode=loose", fixture->fixture_directory, fixture->manifest_path};
    int rc = cov_run_with_args(args, 4, NULL, 0, NULL, 0);
    assert_int_equal(rc, 2);
}
...
static void test_main_verify_invalid_format_exits_two(void** state)
{
    ...
    cov_build_basic_tree(fixture->fixture_directory);
    assert_int_equal(cov_run_manifest_args(fixture->fixture_directory, fixture->manifest_path, NULL, 0), 0);
    const char* args[] = {"verify", "--format=xml", fixture->fixture_directory, fixture->manifest_path};
    int rc = cov_run_with_args(args, 4, NULL, 0, NULL, 0);
    assert_int_equal(rc, 2);
}
...
static void test_main_verify_invalid_threads_exits_two(void** state)
{
    ...
    cov_build_basic_tree(fixture->fixture_directory);
    assert_int_equal(cov_run_manifest_args(fixture->fixture_directory, fixture->manifest_path, NULL, 0), 0);
    const char* args[] = {"--threads=banana", "verify", "--default-exclude-virtual=false", fixture->fixture_directory,
                          fixture->manifest_path};
    int rc = cov_run_with_args(args, 5, NULL, 0, NULL, 0);
    assert_int_equal(rc, 2);
}
...
static void test_main_diff_invalid_format_exits_two(void** state)
{
    ...
    assert_int_equal(cov_run_manifest_args(fixture->fixture_directory, fixture->manifest_path, NULL, 0), 0);
    assert_int_equal(cov_run_manifest_args(fixture->fixture_directory, fixture->manifest_path_b, NULL, 0), 0);
    const char* args[] = {"diff", "--format=xml", fixture->manifest_path, fixture->manifest_path_b};
    int rc = cov_run_with_args(args, 4, NULL, 0, NULL, 0);
    assert_int_equal(rc, 2);
}
...
int main(void)
{
    const struct CMUnitTest tests[] = {
        cmocka_unit_test_setup_teardown(test_main_help_global_exit_zero, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_main_help_per_subcommand, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_main_version_exit_zero, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_main_unknown_subcommand_exits_two, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_main_no_subcommand_exits_two, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_main_manifest_threads_zero_singlethreaded, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_main_manifest_threads_one, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_main_manifest_threads_auto_io, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_main_manifest_threads_exceeds_logical, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_main_manifest_digest_xxh3, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_main_manifest_digest_xxh128, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_main_manifest_with_include_glob, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_main_manifest_with_exclude_glob, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_main_manifest_include_hidden, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_main_manifest_follow_symlinks, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_main_manifest_root_inexistent_exits_one, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_main_manifest_root_is_file_exits_one, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_main_manifest_missing_output_arg_exits_two, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_main_manifest_extra_positional_exits_two, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_main_diff_missing_args_exits_two, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_main_diff_one_arg_exits_two, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_main_diff_two_identical_manifests_exits_zero, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_main_verify_missing_args_exits_two, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_main_invalid_threads_value_exits_two, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_main_invalid_digest_algorithm_exits_two, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_main_verify_threads_zero_singlethreaded, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_main_verify_meta_mode_no_rehash, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_main_verify_content_mode_compares_only_digest, coverage_d_setup, coverage_d_teardown),
        ...
        cmocka_unit_test_setup_teardown(test_diff_kind_change_file_to_dir, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_diff_ignore_mtime_only_mtime_diff_zero, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_diff_invalid_manifest_b_returns_two, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_diff_json_format_added_only, coverage_d_setup, coverage_d_teardown),

        cmocka_unit_test_setup_teardown(test_writer_path_with_dot_prefix_uses_quoting, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_writer_path_with_slash_uses_quoting, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_writer_path_simple_no_quoting, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_writer_summary_with_walltime_set, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_writer_with_no_host_uses_unknown, coverage_d_setup, coverage_d_teardown),

        cmocka_unit_test_setup_teardown(test_capture_empty_file, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_capture_broken_symlink, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_capture_compute_digest_unreadable_file, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test(test_capture_set_error_message_basic),
        cmocka_unit_test(test_capture_kind_name_round_trip_all_kinds),
        cmocka_unit_test_setup_teardown(test_capture_fifo_kind_via_stat, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_capture_socket_kind_via_stat, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_capture_long_link_target, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_capture_compute_digest_missing_file, coverage_d_setup, coverage_d_teardown),

        cmocka_unit_test_setup_teardown(test_walk_serial_with_include_filter, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_walk_serial_with_exclude_filter, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_walk_serial_with_include_special, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_walk_serial_with_hidden_dir_skipped, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_walk_serial_root_inexistent_returns_false, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_walk_serial_virtual_root_blocked, coverage_d_setup, coverage_d_teardown),

        cmocka_unit_test_setup_teardown(test_verify_meta_only_mtime_change, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_verify_zero_changes_json, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_verify_many_entries_consistent, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_verify_path_prefix_relation, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_verify_socket_entry_in_manifest, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_verify_fifo_entry_in_manifest, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_verify_entry_removed_from_filesystem, coverage_d_setup, coverage_d_teardown),

        cmocka_unit_test_setup_teardown(test_cli_bind_manifest_missing_threads_returns_false, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_cli_bind_verify_missing_threads_returns_false, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_cli_bind_diff_missing_format_returns_false, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_cli_bind_manifest_via_full_parse_succeeds, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_cli_bind_verify_via_full_parse_succeeds, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_cli_bind_diff_via_full_parse_succeeds, coverage_d_setup, coverage_d_teardown),
        ...
        cmocka_unit_test_setup_teardown(test_cli_bind_manifest_invalid_threads_emits_invalid, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_cli_bind_verify_invalid_threads_emits_invalid, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_cli_bind_manifest_too_few_positional, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_cli_bind_verify_too_few_positional, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_cli_bind_diff_too_few_positional, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_cli_bind_manifest_with_include_exclude_lists, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_cli_bind_verify_with_include_exclude_lists, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_cli_bind_diff_with_ignore_flags, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_main_diff_format_json_with_changes, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_main_diff_ignore_meta_zero, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_main_diff_with_text_format_explicit, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_main_diff_invalid_manifest_a_exits_two, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_main_verify_invalid_mode_exits_two, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_main_verify_invalid_format_exits_two, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_main_verify_invalid_threads_exits_two, coverage_d_setup, coverage_d_teardown),
        cmocka_unit_test_setup_teardown(test_main_diff_invalid_format_exits_two, coverage_d_setup, coverage_d_teardown),
    };
