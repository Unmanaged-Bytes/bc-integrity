...
static int fixture_setup(void **state) {
  ...
  snprintf(fixture->fixture_directory, sizeof(fixture->fixture_directory),
           "/tmp/bc_integrity_e2e_%d_XXXXXX", getpid());
...
static int run_bc_integrity(const char *root, const char *output,
  ...
  if (pid == 0) {
    ...
    snprintf(default_exclude_argument, sizeof(default_exclude_argument),
             "--default-exclude-virtual=%s",
             default_exclude_virtual ? "true" : "false");
    char *argv[] = {
        (char *)BC_INTEGRITY_TEST_BINARY_PATH,
        "manifest",
        output_argument,
        default_exclude_argument,
        (char *)root,
        NULL,
    };
    execv(argv[0], argv);
...
static void test_e2e_manifest_includes_files_dirs_symlinks(void **state) {
  ...
  int exit_code = run_bc_integrity(fixture->fixture_directory,
                                   fixture->manifest_path, false);
  ...
  assert_true(
      bc_hrbl_reader_open(memory_context, fixture->manifest_path, &reader));

  bc_hrbl_value_ref_t value_ref;

  assert_true(bc_hrbl_reader_find(reader, "entries.'file.txt'.kind",
                                  strlen("entries.'file.txt'.kind"),
                                  &value_ref));
  ...
  assert_true(bc_hrbl_reader_find(reader, "entries.'link.txt'.kind",
                                  strlen("entries.'link.txt'.kind"),
                                  &value_ref));
  assert_true(bc_hrbl_reader_get_string(&value_ref, &kind, &kind_length));
  ...
  assert_true(bc_hrbl_reader_find(reader, "entries.sub.kind",
                                  strlen("entries.sub.kind"), &value_ref));
  assert_true(bc_hrbl_reader_get_string(&value_ref, &kind, &kind_length));
  assert_memory_equal(kind, "dir", 3);

  assert_true(bc_hrbl_reader_find(reader, "entries.'sub/regular.txt'.kind",
                                  strlen("entries.'sub/regular.txt'.kind"),
                                  &value_ref));
  assert_true(bc_hrbl_reader_get_string(&value_ref, &kind, &kind_length));
  assert_memory_equal(kind, "file", 4);

  assert_false(bc_hrbl_reader_find(reader, "entries.'.hidden'.kind",
                                   strlen("entries.'.hidden'.kind"),
                                   &value_ref));
  assert_false(bc_hrbl_reader_find(reader, "entries.'.hidden/file.txt'.kind",
                                   strlen("entries.'.hidden/file.txt'.kind"),
                                   &value_ref));
...
int main(void) {
  const struct CMUnitTest tests[] = {
      cmocka_unit_test_setup_teardown(
          test_e2e_manifest_includes_files_dirs_symlinks, fixture_setup,
          fixture_teardown),
  };
