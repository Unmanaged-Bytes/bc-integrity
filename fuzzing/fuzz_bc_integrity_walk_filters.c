...
static int bc_integrity_fuzz_walk_filters_one(const uint8_t* data, size_t size)
{
    ...
    (void)bc_integrity_walk_is_virtual_subpath(buffer, path_length, buffer, path_length);

    if (path_length > 1u) {
        size_t split = path_length / 2u;
        (void)bc_integrity_walk_is_virtual_subpath(buffer, split, buffer + split, path_length - split);
        (void)bc_integrity_walk_is_virtual_subpath(buffer + split, path_length - split, buffer, split);
    }
