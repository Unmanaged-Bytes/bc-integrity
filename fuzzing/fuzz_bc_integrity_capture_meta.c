...
static const mode_t bc_integrity_fuzz_capture_safe_modes[] = {
    S_IFDIR | 0755u, S_IFIFO | 0644u, S_IFSOCK | 0600u, S_IFBLK | 0600u, S_IFCHR | 0600u,
};

static const size_t bc_integrity_fuzz_capture_safe_mode_count =
    sizeof(bc_integrity_fuzz_capture_safe_modes) / sizeof(bc_integrity_fuzz_capture_safe_modes[0]);
...
static int bc_integrity_fuzz_capture_meta_one(const uint8_t* data, size_t size)
{
    ...
    (void)bc_integrity_capture_entry_from_stat(memory, &stat_buffer, BC_INTEGRITY_DIGEST_ALGORITHM_SHA256, -1, NULL, absolute_path,
                                               absolute_length, relative_path, relative_length, true, &entry);
    ...
    for (bc_integrity_entry_kind_t kind = BC_INTEGRITY_ENTRY_KIND_FILE; kind <= BC_INTEGRITY_ENTRY_KIND_DEVICE;
