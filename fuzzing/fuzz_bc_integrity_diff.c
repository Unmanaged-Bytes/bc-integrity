...
static bool bc_integrity_fuzz_diff_write_temp(const char *suffix,
                                              const uint8_t *data, size_t size,
                                              char *out_path,
                                              size_t out_path_size) {
  int written =
      snprintf(out_path, out_path_size,
               "/tmp/bc_integrity_fuzz_diff_%d_%s.hrbl", getpid(), suffix);
...
static int bc_integrity_fuzz_diff_one(const uint8_t *data, size_t size) {
  ...
  uint32_t split = ((uint32_t)data[0]) | ((uint32_t)data[1] << 8) |
  ...
  char path_a[128];
  char path_b[128];
  if (!bc_integrity_fuzz_diff_write_temp("a", side_a, side_a_length, path_a,
                                         sizeof(path_a))) {
    return 0;
  }
  if (!bc_integrity_fuzz_diff_write_temp("b", side_b, side_b_length, path_b,
                                         sizeof(path_b))) {
    unlink(path_a);
    return 0;
  }
