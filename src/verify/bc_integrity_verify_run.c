...
static void bc_integrity_verify_emit_stderr_quoted(const char* prefix, const char* value, const char* suffix)
{
    char buffer[BC_INTEGRITY_VERIFY_STDERR_BUFFER_BYTES];
    bc_core_writer_t writer;
    if (!bc_core_writer_init_standard_error(&writer, buffer, sizeof(buffer))) {
...
static int bc_integrity_verify_path_compare(const char* left, size_t left_length, const char* right, size_t right_length)
{
...
static bool bc_integrity_verify_record_less_than(const void* left_pointer, const void* right_pointer, void* user_data)
{
    (void)user_data;
    const bc_integrity_verify_record_t* left = (const bc_integrity_verify_record_t*)left_pointer;
    const bc_integrity_verify_record_t* right = (const bc_integrity_verify_record_t*)right_pointer;
    return bc_integrity_verify_path_compare(left->relative_path, left->relative_path_length, right->relative_path,
                                            right->relative_path_length) < 0;
}

static bc_integrity_entry_kind_t bc_integrity_verify_kind_from_name(const char* name, size_t length)
{
    if (length == 4) {
        bool is_file = false;
...
static bool bc_integrity_verify_clone_path(bc_allocators_context_t* memory_context, const char* path, size_t length, char** out_copy)
{
    char* copy = NULL;
...
static bool bc_integrity_verify_field_name_equals(const char* key, size_t key_length, const char* literal, size_t literal_length)
{
...
static bool bc_integrity_verify_load_record_from_block(bc_allocators_context_t* memory_context, const bc_hrbl_value_ref_t* entry_block,
                                                       const char* relative_path, size_t relative_path_length,
                                                       bc_integrity_verify_record_t* out_record)
{
    ...
    while (bc_hrbl_iter_next(&iter, &value, &key, &key_length)) {
        ...
        if (bc_integrity_verify_field_name_equals(key, key_length, "kind", 4)) {
            ...
            if (bc_hrbl_reader_get_string(&value, &kind_value, &kind_length)) {
                out_record->snapshot.kind = bc_integrity_verify_kind_from_name(kind_value, kind_length);
                kind_seen = true;
            }
            continue;
        }
        if (bc_integrity_verify_field_name_equals(key, key_length, "size_bytes", 10)) {
        ...
        if (bc_integrity_verify_field_name_equals(key, key_length, "mtime_sec", 9)) {
            uint64_t value_u64 = 0;
            if (bc_hrbl_reader_get_uint64(&value, &value_u64)) {
        ...
        if (bc_integrity_verify_field_name_equals(key, key_length, "mtime_nsec", 10)) {
            uint64_t value_u64 = 0;
            if (bc_hrbl_reader_get_uint64(&value, &value_u64)) {
        ...
        if (bc_integrity_verify_field_name_equals(key, key_length, "digest_hex", 10)) {
            const char* string_value = NULL;
            size_t string_length = 0;
            if (bc_hrbl_reader_get_string(&value, &string_value, &string_length)) {
                char* copy = NULL;
                if (!bc_integrity_verify_clone_path(memory_context, string_value, string_length, &copy)) {
                    return false;
                }
        ...
        if (bc_integrity_verify_field_name_equals(key, key_length, "link_target", 11)) {
            const char* string_value = NULL;
            size_t string_length = 0;
            if (bc_hrbl_reader_get_string(&value, &string_value, &string_length)) {
                char* copy = NULL;
...
static bool bc_integrity_verify_block_has_record_fields(const bc_hrbl_value_ref_t* block)
{
    bc_hrbl_iter_t iter;
...
static bool bc_integrity_verify_collect_entries_recursive(bc_allocators_context_t* memory_context, const bc_hrbl_value_ref_t* block,
                                                          ...
                                                          bc_containers_vector_t* destination)
{
    ...
    while (bc_hrbl_iter_next(&iter, &value, &key, &key_length)) {
        ...
        if (!bc_integrity_verify_block_has_record_fields(&value)) {
            if (!bc_integrity_verify_collect_entries_recursive(memory_context, &value, combined, combined_length, destination)) {
        ...
        if (!bc_integrity_verify_load_record_from_block(memory_context, &value, combined, combined_length, &record)) {
            bc_allocators_pool_free(memory_context, combined);
            return false;
        }
...
static bool bc_integrity_verify_collect_entries(bc_allocators_context_t* memory_context, const bc_hrbl_reader_t* reader,
                                                bc_containers_vector_t* destination)
{
    ...
    return bc_integrity_verify_collect_entries_recursive(memory_context, &entries_ref, NULL, 0, destination);
}

static bc_integrity_digest_algorithm_t bc_integrity_verify_load_algorithm(const bc_hrbl_reader_t* reader)
{
    bc_hrbl_value_ref_t value_ref;
    if (!bc_hrbl_reader_find(reader, "meta.digest_algorithm", sizeof("meta.digest_algorithm") - 1u, &value_ref)) {
...
static bc_integrity_verify_change_kind_t bc_integrity_verify_compare_one(const bc_integrity_verify_options_t* options,
...
static const char* bc_integrity_verify_mode_label(bc_integrity_verify_mode_t mode)
{
    switch (mode) {
    case BC_INTEGRITY_VERIFY_MODE_CONTENT:
        return "content";
...
static const char* bc_integrity_verify_algorithm_label(bc_integrity_digest_algorithm_t algorithm)
{
    switch (algorithm) {
...
static void bc_integrity_verify_tally_change(bc_integrity_verify_change_kind_t kind, bc_integrity_verify_json_summary_t* summary)
{
    switch (kind) {
...
bool bc_integrity_verify_run(bc_allocators_context_t* memory_context, bc_concurrency_context_t* concurrency_context,
                             bc_runtime_signal_handler_t* signal_handler, const bc_integrity_verify_options_t* options,
                             bc_runtime_error_collector_t* errors, int* out_exit_code)
{
    (void)signal_handler;
    bc_hrbl_verify_status_t verify_status = bc_hrbl_verify_file(options->manifest_path);
    if (verify_status != BC_HRBL_VERIFY_OK) {
        bc_integrity_verify_emit_stderr_quoted("bc-integrity: verify: invalid manifest '", options->manifest_path, "'\n");
    ...
    if (realpath(options->root_path, canonical_root) == NULL) {
        bc_integrity_verify_emit_stderr_quoted("bc-integrity: verify: cannot resolve root '", options->root_path, "'\n");
        *out_exit_code = BC_INTEGRITY_VERIFY_EXIT_ERROR;
        return true;
    }
    size_t canonical_root_length = 0;
    (void)bc_core_length(canonical_root, '\0', &canonical_root_length);
    while (canonical_root_length > 1 && canonical_root[canonical_root_length - 1] == '/') {
        canonical_root[--canonical_root_length] = '\0';
    }

    struct stat root_stat;
    if (stat(canonical_root, &root_stat) != 0 || !S_ISDIR(root_stat.st_mode)) {
        bc_integrity_verify_emit_stderr_quoted("bc-integrity: verify: root must be a directory: '", canonical_root, "'\n");
        *out_exit_code = BC_INTEGRITY_VERIFY_EXIT_ERROR;
        return true;
    }
    ...
    if (!bc_hrbl_reader_open(memory_context, options->manifest_path, &reader)) {
        bc_integrity_verify_emit_stderr_quoted("bc-integrity: verify: cannot open manifest '", options->manifest_path, "'\n");
        *out_exit_code = BC_INTEGRITY_VERIFY_EXIT_ERROR;
        return true;
    }

    bc_integrity_digest_algorithm_t digest_algorithm = bc_integrity_verify_load_algorithm(reader);

    bc_containers_vector_t* expected_records = NULL;
    if (!bc_containers_vector_create(memory_context, sizeof(bc_integrity_verify_record_t), BC_INTEGRITY_VERIFY_INITIAL_VECTOR_CAPACITY,
    ...
    if (!bc_integrity_verify_collect_entries(memory_context, reader, expected_records)) {
        bc_containers_vector_destroy(memory_context, expected_records);
        bc_hrbl_reader_close(reader);
        bc_integrity_verify_emit_stderr("bc-integrity: verify: failed to enumerate manifest entries\n");
        *out_exit_code = BC_INTEGRITY_VERIFY_EXIT_ERROR;
        return true;
    }

    bc_containers_vector_t* actual_entries = NULL;
    if (!bc_containers_vector_create(memory_context, sizeof(bc_integrity_entry_t), BC_INTEGRITY_VERIFY_INITIAL_VECTOR_CAPACITY,
                                     BC_INTEGRITY_VERIFY_MAX_VECTOR_CAPACITY, &actual_entries)) {
        bc_containers_vector_destroy(memory_context, expected_records);
        bc_hrbl_reader_close(reader);
        *out_exit_code = BC_INTEGRITY_VERIFY_EXIT_ERROR;
        return true;
    ...
    bool walk_ok = bc_integrity_walk_run(memory_context, concurrency_context, signal_handler, &walk_options, canonical_root,
                                         canonical_root_length, actual_entries, errors);
    if (!walk_ok) {
    ...
    if (walk_options.defer_digest) {
        if (!bc_integrity_dispatch_compute_digests(memory_context, concurrency_context, signal_handler, digest_algorithm, actual_entries)) {
            bc_runtime_error_collector_flush_to_stderr(errors, "bc-integrity");
            bc_containers_vector_destroy(memory_context, actual_entries);
            bc_containers_vector_destroy(memory_context, expected_records);
            bc_hrbl_reader_close(reader);
    ...
    for (size_t index = 0; index < actual_count; ++index) {
        ...
        if (entry.digest_hex_length > 0) {
            ...
            if (!bc_integrity_verify_clone_path(memory_context, entry.digest_hex, entry.digest_hex_length, &digest_copy)) {
    ...
    if (expected_count > 0) {
        if (!bc_allocators_pool_allocate(memory_context, expected_count * sizeof(bc_integrity_verify_record_t), (void**)&expected_array)) {
            bc_containers_vector_destroy(memory_context, actual_records);
            bc_containers_vector_destroy(memory_context, actual_entries);
            bc_containers_vector_destroy(memory_context, expected_records);
            bc_hrbl_reader_close(reader);
        ...
        for (size_t index = 0; index < expected_count; ++index) {
            (void)bc_containers_vector_get(expected_records, index, &expected_array[index]);
        }
    }
    ...
    if (actual_record_count > 0) {
        if (!bc_allocators_pool_allocate(memory_context, actual_record_count * sizeof(bc_integrity_verify_record_t),
                                         (void**)&actual_array)) {
        ...
        for (size_t index = 0; index < actual_record_count; ++index) {
            (void)bc_containers_vector_get(actual_records, index, &actual_array[index]);
        }
    }

    if (expected_count > 1) {
        bc_core_sort_with_compare(expected_array, expected_count, sizeof(bc_integrity_verify_record_t),
                                  bc_integrity_verify_record_less_than, NULL);
    }
    if (actual_record_count > 1) {
        bc_core_sort_with_compare(actual_array, actual_record_count, sizeof(bc_integrity_verify_record_t),
                                  bc_integrity_verify_record_less_than, NULL);
    }
    ...
    if (stdout_writer_ready && options->format == BC_INTEGRITY_OUTPUT_FORMAT_JSON) {
        ...
        header_options.digest_algorithm = bc_integrity_verify_algorithm_label(digest_algorithm);
    ...
    while (cursor_expected < expected_count && cursor_actual < actual_record_count && !exit_early) {
        const bc_integrity_verify_record_t* expected_record = &expected_array[cursor_expected];
        const bc_integrity_verify_record_t* actual_record = &actual_array[cursor_actual];
        int comparison = bc_integrity_verify_path_compare(expected_record->relative_path, expected_record->relative_path_length,
        ...
        if (comparison < 0) {
            if (stdout_writer_ready) {
                if (options->format == BC_INTEGRITY_OUTPUT_FORMAT_JSON) {
                    (void)bc_integrity_verify_emit_change_json(&stdout_writer, BC_INTEGRITY_VERIFY_CHANGE_REMOVED,
                                                               expected_record->relative_path, expected_record->relative_path_length,
                                                               &expected_record->snapshot, NULL);
                } else {
                    (void)bc_integrity_verify_emit_change_text(&stdout_writer, BC_INTEGRITY_VERIFY_CHANGE_REMOVED,
                                                               expected_record->relative_path, expected_record->relative_path_length,
                                                               &expected_record->snapshot, NULL);
                }
            }
            bc_integrity_verify_tally_change(BC_INTEGRITY_VERIFY_CHANGE_REMOVED, &summary);
            change_count += 1;
            cursor_expected += 1;
        } else if (comparison > 0) {
            if (stdout_writer_ready) {
                if (options->format == BC_INTEGRITY_OUTPUT_FORMAT_JSON) {
                    (void)bc_integrity_verify_emit_change_json(&stdout_writer, BC_INTEGRITY_VERIFY_CHANGE_ADDED,
                                                               actual_record->relative_path, actual_record->relative_path_length, NULL,
                                                               &actual_record->snapshot);
                } else {
                    (void)bc_integrity_verify_emit_change_text(&stdout_writer, BC_INTEGRITY_VERIFY_CHANGE_ADDED,
                                                               actual_record->relative_path, actual_record->relative_path_length, NULL,
                                                               &actual_record->snapshot);
                }
            }
            bc_integrity_verify_tally_change(BC_INTEGRITY_VERIFY_CHANGE_ADDED, &summary);
            change_count += 1;
            cursor_actual += 1;
        } else {
            bc_integrity_verify_change_kind_t change =
                bc_integrity_verify_compare_one(options, &expected_record->snapshot, &actual_record->snapshot);
            if (change != BC_INTEGRITY_VERIFY_CHANGE_NONE) {
                if (stdout_writer_ready) {
                    if (options->format == BC_INTEGRITY_OUTPUT_FORMAT_JSON) {
                        (void)bc_integrity_verify_emit_change_json(&stdout_writer, change, expected_record->relative_path,
                                                                   expected_record->relative_path_length, &expected_record->snapshot,
                                                                   &actual_record->snapshot);
                    } else {
                        (void)bc_integrity_verify_emit_change_text(&stdout_writer, change, expected_record->relative_path,
                                                                   expected_record->relative_path_length, &expected_record->snapshot,
                                                                   &actual_record->snapshot);
                    }
                }
    ...
    while (cursor_actual < actual_record_count && !exit_early) {
        ...
        if (stdout_writer_ready) {
            if (options->format == BC_INTEGRITY_OUTPUT_FORMAT_JSON) {
                (void)bc_integrity_verify_emit_change_json(&stdout_writer, BC_INTEGRITY_VERIFY_CHANGE_ADDED, actual_record->relative_path,
                                                           actual_record->relative_path_length, NULL, &actual_record->snapshot);
            } else {
                (void)bc_integrity_verify_emit_change_text(&stdout_writer, BC_INTEGRITY_VERIFY_CHANGE_ADDED, actual_record->relative_path,
                                                           actual_record->relative_path_length, NULL, &actual_record->snapshot);
            }
        }
        bc_integrity_verify_tally_change(BC_INTEGRITY_VERIFY_CHANGE_ADDED, &summary);
    ...
    *out_exit_code = (change_count == 0) ? BC_INTEGRITY_VERIFY_EXIT_OK : BC_INTEGRITY_VERIFY_EXIT_DIFF;
    return true;
}
