...
static bool bc_integrity_verify_strict_strings_equal(const char *left,
                                                     size_t left_length,
                                                     const char *right,
                                                     size_t right_length) {
...
static bool bc_integrity_verify_strict_meta_matches(
  ...
  if (!bc_integrity_verify_strict_strings_equal(
...
static bool bc_integrity_verify_strict_digest_matches(
    const bc_integrity_meta_snapshot_t *expected,
    const bc_integrity_meta_snapshot_t *actual) {
  return bc_integrity_verify_strict_strings_equal(
      expected->digest_hex, expected->digest_hex_length, actual->digest_hex,
...
bc_integrity_verify_compare_strict(const bc_integrity_meta_snapshot_t *expected,
                                   ...
                                   bool ignore_mtime) {
  bool meta_equal =
      bc_integrity_verify_strict_meta_matches(expected, actual, ignore_mtime);
  bool digest_equal =
      bc_integrity_verify_strict_digest_matches(expected, actual);
