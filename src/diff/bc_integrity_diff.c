...
static void bc_integrity_diff_emit_stderr_quoted(const char *prefix,
                                                 const char *value,
                                                 const char *suffix) {
...
static int bc_integrity_diff_path_compare(const char *left, size_t left_length,
                                          const char *right,
                                          size_t right_length) {
...
static bool bc_integrity_diff_record_less_than(const void *left_pointer,
                                               const void *right_pointer,
                                               void *user_data) {
  (void)user_data;
  const bc_integrity_diff_record_t *left =
      (const bc_integrity_diff_record_t *)left_pointer;
  const bc_integrity_diff_record_t *right =
      (const bc_integrity_diff_record_t *)right_pointer;
  return bc_integrity_diff_path_compare(
...
bc_integrity_diff_kind_from_name(const char *name, size_t length) {
  if (length == 4) {
    bool is_file = false;
...
static bool bc_integrity_diff_path_needs_quoting(const char *path,
                                                 size_t length) {
...
static bool
bc_integrity_diff_clone_string(bc_allocators_context_t *memory_context,
...
static bool bc_integrity_diff_format_entry_key(char *buffer, size_t buffer_size,
                                               const char *path,
                                               size_t path_length,
                                               const char *suffix,
                                               size_t *out_length) {
...
static bool bc_integrity_diff_lookup_uint64(const bc_hrbl_reader_t *reader,
...
static bool bc_integrity_diff_lookup_string(const bc_hrbl_reader_t *reader,
                                            const char *key, size_t key_length,
...
static bool bc_integrity_diff_load_record(
    bc_allocators_context_t *memory_context, const bc_hrbl_reader_t *reader,
    const char *relative_path, size_t relative_path_length,
    bc_integrity_diff_record_t *out_record) {
  ...
  if (!bc_integrity_diff_format_entry_key(key_buffer, sizeof(key_buffer),
  ...
  if (bc_integrity_diff_lookup_string(reader, key_buffer, key_length,
                                      &kind_value, &kind_length)) {
    out_record->snapshot.kind =
        bc_integrity_diff_kind_from_name(kind_value, kind_length);
  }

  uint64_t value_u64 = 0;
  (void)bc_integrity_diff_format_entry_key(key_buffer, sizeof(key_buffer),
                                           relative_path, relative_path_length,
                                           ".size_bytes", &key_length);
  if (bc_integrity_diff_lookup_uint64(reader, key_buffer, key_length,
                                      &value_u64)) {
  ...
  (void)bc_integrity_diff_format_entry_key(key_buffer, sizeof(key_buffer),
                                           relative_path, relative_path_length,
                                           ".digest_hex", &key_length);
  const char *string_value = NULL;
  size_t string_length = 0;
  if (bc_integrity_diff_lookup_string(reader, key_buffer, key_length,
                                      &string_value, &string_length)) {
    char *copy = NULL;
    if (!bc_integrity_diff_clone_string(memory_context, string_value,
                                        string_length, &copy)) {
      return false;
    }
...
static bool bc_integrity_diff_collect_recursive(
    bc_allocators_context_t *memory_context, const bc_hrbl_reader_t *reader,
    const bc_hrbl_value_ref_t *block, const char *path_prefix,
    size_t path_prefix_length, bc_containers_vector_t *destination) {
  bc_hrbl_iter_t iter;
  ...
  while (bc_hrbl_iter_next(&iter, &value, &key, &key_length)) {
    ...
    if (!bc_integrity_diff_load_record(memory_context, reader, combined,
                                       combined_length, &record)) {
...
static bool
bc_integrity_diff_collect_entries(bc_allocators_context_t *memory_context,
  ...
  return bc_integrity_diff_collect_recursive(
      memory_context, reader, &entries_ref, NULL, 0, destination);
}

static bool bc_integrity_diff_load_side(bc_allocators_context_t *memory_context,
                                        const char *manifest_path,
                                        bc_hrbl_reader_t **out_reader,
                                        bc_integrity_diff_record_t **out_array,
                                        size_t *out_count) {
  bc_hrbl_verify_status_t verify_status = bc_hrbl_verify_file(manifest_path);
  if (verify_status != BC_HRBL_VERIFY_OK) {
    bc_integrity_diff_emit_stderr_quoted(
        "bc-integrity: diff: invalid manifest '", manifest_path, "'\n");
    return false;
  }
  if (!bc_hrbl_reader_open(memory_context, manifest_path, out_reader)) {
    bc_integrity_diff_emit_stderr_quoted(
        "bc-integrity: diff: cannot open manifest '", manifest_path, "'\n");
    return false;
  }
  ...
  bc_containers_vector_destroy(memory_context, vector);
  if (count > 1) {
    bc_core_sort_with_compare(array, count, sizeof(bc_integrity_diff_record_t),
                              bc_integrity_diff_record_less_than, NULL);
  }
  *out_array = array;
  *out_count = count;
  return true;
...
bc_integrity_diff_classify(const bc_integrity_diff_options_t *options,
                           const bc_integrity_meta_snapshot_t *expected,
                           const bc_integrity_meta_snapshot_t *actual) {
  bc_integrity_verify_change_kind_t content_change =
      bc_integrity_verify_compare_content(expected, actual);
  bc_integrity_verify_change_kind_t meta_change =
      bc_integrity_verify_compare_meta(expected, actual, options->ignore_mtime);
...
bool bc_integrity_diff_run(bc_allocators_context_t *memory_context,
  ...
  if (!bc_integrity_diff_load_side(memory_context, options->manifest_path_a,
                                   &reader_a, &records_a, &count_a)) {
    *out_exit_code = BC_INTEGRITY_DIFF_EXIT_ERROR;
    return true;
  }
  if (!bc_integrity_diff_load_side(memory_context, options->manifest_path_b,
                                   &reader_b, &records_b, &count_b)) {
    if (records_a != NULL) {
  ...
  while (cursor_a < count_a && cursor_b < count_b) {
    ...
    int comparison = bc_integrity_diff_path_compare(
        left->relative_path, left->relative_path_length, right->relative_path,
    ...
    if (comparison < 0) {
      if (stdout_writer_ready) {
        if (options->format == BC_INTEGRITY_OUTPUT_FORMAT_JSON) {
          (void)bc_integrity_verify_emit_change_json(
              ...
              left->relative_path, left->relative_path_length, &left->snapshot,
              NULL);
        } else {
      ...
      if (stdout_writer_ready) {
        if (options->format == BC_INTEGRITY_OUTPUT_FORMAT_JSON) {
          (void)bc_integrity_verify_emit_change_json(
              ...
              right->relative_path, right->relative_path_length, NULL,
              &right->snapshot);
        } else {
          (void)bc_integrity_verify_emit_change_text(
              &stdout_writer, BC_INTEGRITY_VERIFY_CHANGE_ADDED,
      ...
      bc_integrity_verify_change_kind_t change = bc_integrity_diff_classify(
          options, &left->snapshot, &right->snapshot);
      if (change != BC_INTEGRITY_VERIFY_CHANGE_NONE) {
        if (stdout_writer_ready) {
          if (options->format == BC_INTEGRITY_OUTPUT_FORMAT_JSON) {
            (void)bc_integrity_verify_emit_change_json(
                &stdout_writer, change, left->relative_path,
                left->relative_path_length, &left->snapshot, &right->snapshot);
          } else {
            (void)bc_integrity_verify_emit_change_text(
                &stdout_writer, change, left->relative_path,
                left->relative_path_length, &left->snapshot, &right->snapshot);
