...
static const size_t bc_integrity_walk_virtual_top_level_count =
    sizeof(bc_integrity_walk_virtual_top_level_paths) / sizeof(bc_integrity_walk_virtual_top_level_paths[0]);

static bool bc_integrity_walk_path_starts_with(const char* path, size_t path_length, const char* prefix, size_t prefix_length)
{
...
{
    for (size_t index = 0; index < bc_integrity_walk_virtual_top_level_count; ++index) {
        ...
        if (canonical_root_path_length == prefix_length &&
            bc_integrity_walk_path_starts_with(canonical_root_path, canonical_root_path_length, prefix, prefix_length)) {
            return true;
        }
    }
    return false;
...
static bool bc_integrity_walk_path_is_descendant_or_equal(const char* path, size_t path_length, const char* prefix, size_t prefix_length)
{
    if (path_length < prefix_length) {
        return false;
    }
    if (!bc_integrity_walk_path_starts_with(path, path_length, prefix, prefix_length)) {
        return false;
    }
    if (path_length == prefix_length) {
        return true;
                                          ...
                                          size_t absolute_path_length)
{
    ...
    for (size_t index = 0; index < bc_integrity_walk_virtual_top_level_count; ++index) {
        ...
        if (bc_integrity_walk_path_is_descendant_or_equal(canonical_root_path, canonical_root_path_length, prefix, prefix_length)) {
            continue;
        }

        if (bc_integrity_walk_path_is_descendant_or_equal(absolute_path, absolute_path_length, prefix, prefix_length)) {
            return true;
        }
    }
    return false;
