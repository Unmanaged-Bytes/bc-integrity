...
static bool bc_integrity_walk_relative_path(bc_allocators_context_t* memory_context, const char* canonical_root_path,
                                            size_t canonical_root_path_length, const char* absolute_path, size_t absolute_path_length,
                                            const char** out_relative_path, size_t* out_relative_path_length)
{
...
static bool bc_integrity_walk_ensure_slot(const bc_integrity_walk_context_t* context, bc_allocators_context_t* worker_memory,
                                          bc_integrity_walk_worker_slot_t** out_slot)
{
    bc_integrity_walk_worker_slot_t* slot = (bc_integrity_walk_worker_slot_t*)bc_concurrency_worker_slot(context->worker_slot_index);
    if (slot == NULL) {
        return false;
    }
    if (!slot->initialized) {
        if (!bc_containers_vector_create(worker_memory, sizeof(bc_integrity_entry_t), BC_INTEGRITY_WALK_INITIAL_VECTOR_CAPACITY,
...
static int bc_integrity_walk_open_parent_directory(const char* absolute_path, size_t absolute_path_length, size_t* out_basename_offset)
{
    size_t last_slash_offset = 0;
    if (!bc_core_find_last_byte(absolute_path, absolute_path_length, '/', &last_slash_offset)) {
...
static bool bc_integrity_walk_clone_path(bc_allocators_context_t* memory_context, const char* path, size_t path_length,
                                         const char** out_copy)
{
    char* copy = NULL;
...
static bool bc_integrity_walk_capture_for_entry(const bc_integrity_walk_context_t* context, bc_allocators_context_t* worker_memory,
                                                bc_integrity_walk_worker_slot_t* slot, const bc_io_walk_entry_t* entry,
                                                bool follow_for_stat)
{
    size_t basename_offset = 0;
    int parent_directory_fd = bc_integrity_walk_open_parent_directory(entry->absolute_path, entry->absolute_path_length, &basename_offset);
    if (parent_directory_fd < 0) {
        (void)bc_runtime_error_collector_append(slot->errors, worker_memory, entry->absolute_path, "open-parent", errno);
        return false;
    }
    ...
    if (fstatat(parent_directory_fd, basename, &stat_buffer, stat_flags) != 0) {
        (void)bc_runtime_error_collector_append(slot->errors, worker_memory, entry->absolute_path, "lstat", errno);
    ...
    if (!bc_integrity_walk_relative_path(worker_memory, context->canonical_root_path, context->canonical_root_path_length,
                                         entry->absolute_path, entry->absolute_path_length, &relative_path, &relative_path_length)) {
        (void)bc_runtime_error_collector_append(slot->errors, worker_memory, entry->absolute_path, "relative-path", EINVAL);
        close(parent_directory_fd);
        return false;
    }
    ...
    if (!bc_integrity_walk_clone_path(worker_memory, entry->absolute_path, entry->absolute_path_length, &absolute_path_copy)) {
        (void)bc_runtime_error_collector_append(slot->errors, worker_memory, entry->absolute_path, "clone-absolute", ENOMEM);
        close(parent_directory_fd);
        return false;
    }
    ...
    bc_integrity_capture_entry_from_stat(worker_memory, &stat_buffer, context->options->digest_algorithm, parent_directory_fd, basename,
                                         entry->absolute_path, entry->absolute_path_length, relative_path, relative_path_length,
    ...
    if (!bc_containers_vector_push(worker_memory, slot->entries, &built_entry)) {
        (void)bc_runtime_error_collector_append(slot->errors, worker_memory, entry->absolute_path, "vector-push", ENOMEM);
        return false;
    }
    return true;
}
...
static bool bc_integrity_walk_filter(const bc_io_walk_entry_t* entry, void* user_data)
{
    bc_integrity_walk_context_t* context = (bc_integrity_walk_context_t*)user_data;
    ...
    if (entry->kind == BC_IO_WALK_ENTRY_SYMLINK) {
        ...
        (void)bc_integrity_walk_capture_for_entry(context, worker_memory, slot, entry, false);
        return false;
    }
    ...
    if (context->options->default_exclude_virtual &&
        bc_integrity_walk_is_virtual_subpath(context->canonical_root_path, context->canonical_root_path_length, entry->absolute_path,
...
// cppcheck-suppress constParameterCallback ; signature fixed by
// bc_io_walk_visit_fn
static bool bc_integrity_walk_visit(const bc_io_walk_entry_t* entry, void* user_data)
{
    bc_integrity_walk_context_t* context = (bc_integrity_walk_context_t*)user_data;

    if (entry->kind != BC_IO_WALK_ENTRY_FILE && entry->kind != BC_IO_WALK_ENTRY_DIRECTORY) {
        return true;
    }
    ...
    bool follow_for_stat =
        (entry->kind == BC_IO_WALK_ENTRY_FILE || entry->kind == BC_IO_WALK_ENTRY_DIRECTORY) && context->options->follow_symlinks;
    (void)bc_integrity_walk_capture_for_entry(context, worker_memory, slot, entry, follow_for_stat);
    return true;
}

// cppcheck-suppress constParameterCallback ; signature fixed by
// bc_io_walk_should_descend_fn
static bool bc_integrity_walk_should_descend(const bc_io_walk_entry_t* entry, void* user_data)
{
    const bc_integrity_walk_context_t* context = (const bc_integrity_walk_context_t*)user_data;
    ...
    while (relative_offset < entry->absolute_path_length && entry->absolute_path[relative_offset] == '/') {
        relative_offset += 1u;
    }
    ...
    return bc_integrity_filter_accepts_directory(context->filter, entry->absolute_path + relative_offset, relative_length);
}

// cppcheck-suppress constParameterCallback ; signature fixed by
// bc_io_walk_error_fn
static void bc_integrity_walk_on_error(const char* path, const char* stage, int errno_value, void* user_data)
{
    const bc_integrity_walk_context_t* context = (const bc_integrity_walk_context_t*)user_data;
    bc_allocators_context_t* worker_memory = bc_concurrency_worker_memory();
    if (worker_memory == NULL) {
...
static void bc_integrity_walk_merge_worker_slot(void* slot_data, size_t worker_index, void* arg)
{
    (void)worker_index;
    const bc_integrity_walk_worker_slot_t* slot = (const bc_integrity_walk_worker_slot_t*)slot_data;
    bc_integrity_walk_merge_argument_t* merge_argument = (bc_integrity_walk_merge_argument_t*)arg;
...
static bool bc_integrity_walk_try_serial(bc_allocators_context_t* memory_context, const bc_concurrency_context_t* concurrency_context,
                                         ...
                                         bc_containers_vector_t* destination_entries, bc_runtime_error_collector_t* errors,
                                         bool* out_completed)
{
    *out_completed = false;
    size_t worker_count = bc_concurrency_effective_worker_count(concurrency_context);
    if (worker_count <= 1) {
        if (!bc_integrity_walk_run_serial(memory_context, signal_handler, options, canonical_root_path, canonical_root_path_length,
    ...
    if (!bc_integrity_walk_run_serial_with_budget(memory_context, signal_handler, options, canonical_root_path, canonical_root_path_length,
                                                  destination_entries, errors, budget, &budget_exceeded)) {
        return false;
    }
    if (budget_exceeded) {
                           ...
                           bc_runtime_error_collector_t* errors)
{
    ...
    if (!bc_integrity_walk_try_serial(memory_context, concurrency_context, signal_handler, options, canonical_root_path,
                                      canonical_root_path_length, destination_entries, errors, &serial_completed)) {
        return false;
    }
    if (serial_completed) {
        return true;
    ...
    if (!bc_concurrency_register_slot(concurrency_context, &slot_config, &context.worker_slot_index)) {
        (void)bc_runtime_error_collector_append(errors, memory_context, canonical_root_path, "register-slot", ENOMEM);
        return false;
    }
    ...
    bc_concurrency_foreach_slot(concurrency_context, context.worker_slot_index, bc_integrity_walk_merge_worker_slot, &merge_argument);
