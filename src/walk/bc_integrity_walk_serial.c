...
static bool bc_integrity_walk_serial_should_stop(
    const bc_integrity_walk_serial_context_t *context) {
...
static bool bc_integrity_walk_serial_check_budget(
...
static bool bc_integrity_walk_serial_relative_path(
...
static bool bc_integrity_walk_serial_clone_path(
    bc_allocators_context_t *memory_context, const char *path,
    size_t path_length, const char **out_copy) {
  char *copy = NULL;
...
static bool bc_integrity_walk_serial_capture_entry(
    bc_integrity_walk_serial_context_t *context, int parent_directory_fd,
    const char *child_name, const char *absolute_path,
    size_t absolute_path_length, bool follow_for_stat) {
  ...
  if (fstatat(parent_directory_fd, child_name, &stat_buffer, stat_flags) != 0) {
    (void)bc_runtime_error_collector_append(context->errors,
                                            ...
                                            absolute_path, "lstat", errno);
    return false;
  }

  const char *relative_path = NULL;
  size_t relative_path_length = 0;
  if (!bc_integrity_walk_serial_relative_path(
          context->memory_context, context->canonical_root_path,
          context->canonical_root_path_length, absolute_path,
          absolute_path_length, &relative_path, &relative_path_length)) {
    (void)bc_runtime_error_collector_append(context->errors,
                                            context->memory_context,
  ...
  if (!bc_integrity_walk_serial_clone_path(context->memory_context,
                                           absolute_path, absolute_path_length,
                                           &absolute_path_copy)) {
    (void)bc_runtime_error_collector_append(context->errors,
                                            context->memory_context,
                                            absolute_path, "clone-absolute",
                                            ENOMEM);
    return false;
  }
  ...
  bc_integrity_capture_entry_from_stat(
      context->memory_context, &stat_buffer, context->options->digest_algorithm,
      parent_directory_fd, child_name, absolute_path, absolute_path_length,
      relative_path, relative_path_length, defer_digest, &built_entry);
  built_entry.absolute_path = absolute_path_copy;
  built_entry.absolute_path_length = absolute_path_length;

  if (!bc_containers_vector_push(context->memory_context,
                                 context->destination_entries, &built_entry)) {
    (void)bc_runtime_error_collector_append(context->errors,
                                            context->memory_context,
                                            absolute_path, "vector-push",
                                            ENOMEM);
    return false;
  }
...
static bool bc_integrity_walk_serial_should_descend_directory(
    const bc_integrity_walk_serial_context_t *context,
  ...
  return bc_integrity_filter_accepts_directory(
      context->filter, absolute_path + relative_offset, relative_length);
}

static bool bc_integrity_walk_serial_recurse(
    bc_integrity_walk_serial_context_t *context, int directory_fd,
    const char *directory_path, size_t directory_path_length);

static bool bc_integrity_walk_serial_handle_dirent(
    bc_integrity_walk_serial_context_t *context, int directory_fd,
    const char *directory_path, size_t directory_path_length,
  ...
  if (directory_path_length + 1u + entry->name_length + 1u >
      BC_INTEGRITY_WALK_SERIAL_PATH_BUFFER_BYTES) {
    (void)bc_runtime_error_collector_append(context->errors,
                                            context->memory_context,
                                            directory_path, "path-too-long",
                                            ENAMETOOLONG);
    return true;
  }
  ...
  if (child_path_length == 0 ||
      child_path[child_path_length - 1u] != '/') {
  ...
  if (d_type == DT_UNKNOWN) {
    ...
    if (fstatat(directory_fd, entry->name, &probe_stat,
  ...
  if (d_type == DT_LNK) {
    return bc_integrity_walk_serial_capture_entry(
        context, directory_fd, entry->name, child_path, child_path_length,
        false);
  }
  if (d_type == DT_REG) {
    return bc_integrity_walk_serial_capture_entry(
  ...
  if (d_type == DT_DIR) {
    ...
    if (!bc_integrity_walk_serial_should_descend_directory(
            context, child_path, child_path_length)) {
      return true;
    }
    (void)bc_integrity_walk_serial_capture_entry(
        context, directory_fd, entry->name, child_path, child_path_length,
        context->options->follow_symlinks);
    ...
    if (child_fd < 0) {
      (void)bc_runtime_error_collector_append(context->errors,
                                              context->memory_context,
                                              child_path, "open", errno);
      return true;
    }
    bool descend_ok = bc_integrity_walk_serial_recurse(
        context, child_fd, child_path, child_path_length);
    close(child_fd);
    return descend_ok;
  }
...
static bool bc_integrity_walk_serial_recurse(
  ...
  while (true) {
    ...
    (void)bc_integrity_walk_serial_handle_dirent(context, directory_fd,
                                                 directory_path,
...
bool bc_integrity_walk_run_serial_with_budget(
    ...
    bc_containers_vector_t *destination_entries,
    bc_runtime_error_collector_t *errors, size_t entry_budget,
  ...
  *out_budget_exceeded = false;
  ...
  if (options->include_list != NULL || options->exclude_list != NULL) {
    if (!bc_integrity_filter_create(memory_context, options->include_list,
      ...
      (void)bc_runtime_error_collector_append(errors, memory_context,
                                              canonical_root_path,
                                              "filter-create", ENOMEM);
      return false;
    }
  }
  ...
  bool walk_ok = bc_integrity_walk_serial_recurse(
      &context, root_fd, canonical_root_path, canonical_root_path_length);
  close(root_fd);
  if (filter != NULL) {
    bc_integrity_filter_destroy(memory_context, filter);
  }
...
bool bc_integrity_walk_run_serial(
  ...
  return bc_integrity_walk_run_serial_with_budget(
      memory_context, signal_handler, options, canonical_root_path,
      canonical_root_path_length, destination_entries, errors, 0,
      &budget_exceeded);
}
