...
#define BC_INTEGRITY_HASH_RING_QUEUE_DEPTH ((unsigned int)(BC_INTEGRITY_HASH_RING_SLOT_COUNT * 5U))
...
typedef struct bc_integrity_hash_ring {
    ...
    bool ring_ready;
    unsigned char slot_buffers[BC_INTEGRITY_HASH_RING_SLOT_COUNT][BC_INTEGRITY_HASH_RING_SLOT_BUFFER_BYTES];
...
bool bc_integrity_hash_ring_init(bc_integrity_hash_ring_t* ring)
{
    ...
    int queue_status = io_uring_queue_init(BC_INTEGRITY_HASH_RING_QUEUE_DEPTH, &ring->ring, 0);
    if (queue_status < 0) {
        return false;
    }
    int register_status = io_uring_register_files_sparse(&ring->ring, BC_INTEGRITY_HASH_RING_SLOT_COUNT);
...
static uint64_t bc_integrity_hash_ring_encode_user_data(unsigned int slot_index, unsigned int operation_code)
{
...
static unsigned int bc_integrity_hash_ring_decode_operation(uint64_t user_data)
{
    return (unsigned int)(user_data & 0xFFU);
}

static bool bc_integrity_hash_ring_submit_slot(struct io_uring* ring, unsigned int slot_index, const bc_integrity_hash_batch_item_t* item,
                                               void* buffer_address, unsigned char* probe_byte_address)
{
    ...
    io_uring_prep_openat_direct(open_sqe, AT_FDCWD, item->absolute_path, open_flags, 0, slot_index);
    open_sqe->flags |= IOSQE_IO_LINK;
    io_uring_sqe_set_data64(open_sqe, bc_integrity_hash_ring_encode_user_data(slot_index, BC_INTEGRITY_HASH_RING_OP_OPENAT));
    ...
    io_uring_sqe_set_data64(read_sqe, bc_integrity_hash_ring_encode_user_data(slot_index, BC_INTEGRITY_HASH_RING_OP_READ));
    ...
    io_uring_sqe_set_data64(probe_sqe, bc_integrity_hash_ring_encode_user_data(slot_index, BC_INTEGRITY_HASH_RING_OP_PROBE));
    io_uring_prep_close_direct(close_sqe, slot_index);
    io_uring_sqe_set_data64(close_sqe, bc_integrity_hash_ring_encode_user_data(slot_index, BC_INTEGRITY_HASH_RING_OP_CLOSE));
    return true;
}

static bool bc_integrity_hash_ring_fits_in_slot(const bc_integrity_hash_batch_item_t* item)
{
    return item->file_size > 0 && item->file_size <= BC_INTEGRITY_HASH_RING_SLOT_BUFFER_BYTES;
}

static bool bc_integrity_hash_ring_handle_cqe(bc_integrity_hash_ring_t* ring, bc_integrity_hash_batch_item_t* items,
                                              const size_t* slot_to_item, size_t* slot_bytes_read,
                                              bc_integrity_hash_consumer_fn_t consumer_function, struct io_uring_cqe* cqe)
{
    uint64_t user_data = io_uring_cqe_get_data64(cqe);
    unsigned int slot_index = bc_integrity_hash_ring_decode_slot(user_data);
    unsigned int operation_code = bc_integrity_hash_ring_decode_operation(user_data);
...
static bool bc_integrity_hash_ring_drive_chunk(bc_integrity_hash_ring_t* ring, bc_integrity_hash_batch_item_t* items, size_t chunk_start,
                                               size_t chunk_count, bc_integrity_hash_consumer_fn_t consumer_function)
{
    ...
    for (size_t offset = 0; offset < chunk_count; ++offset) {
        ...
        if (!bc_integrity_hash_ring_submit_slot(&ring->ring, slot_index, item, ring->slot_buffers[slot_index],
    ...
    while (completions_seen < expected_completion_count) {
        ...
        unsigned int batch_count = io_uring_peek_batch_cqe(&ring->ring, cqe_batch, BC_INTEGRITY_HASH_RING_QUEUE_DEPTH);
        for (unsigned int i = 0; i < batch_count; ++i) {
            bc_integrity_hash_ring_handle_cqe(ring, items, slot_to_item, slot_bytes_read, consumer_function, cqe_batch[i]);
        }
...
static void bc_integrity_hash_ring_fallback_sync(bc_integrity_hash_batch_item_t* item, bc_integrity_hash_consumer_fn_t consumer_function)
{
    int fallback_errno = 0;
    if (bc_integrity_hash_consume_file(item->absolute_path, item->file_size, item->consumer_context, consumer_function, &fallback_errno)) {
        item->success = true;
        item->errno_value = 0;
    } else {
        item->success = false;
...
bool bc_integrity_hash_consume_batch(bc_integrity_hash_ring_t* ring, bc_integrity_hash_batch_item_t* items, size_t item_count,
                                     bc_integrity_hash_consumer_fn_t consumer_function)
{
    ...
    while (processed < item_count) {
        ...
        size_t chunk_count = chunk_remaining < BC_INTEGRITY_HASH_RING_SLOT_COUNT ? chunk_remaining : BC_INTEGRITY_HASH_RING_SLOT_COUNT;
        if (!bc_integrity_hash_ring_drive_chunk(ring, items, processed, chunk_count, consumer_function)) {
            for (size_t offset = 0; offset < chunk_count; ++offset) {
