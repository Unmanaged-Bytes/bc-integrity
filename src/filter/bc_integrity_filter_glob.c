...
static bool bc_integrity_filter_glob_segment(const char* pattern, size_t pattern_length, const char* value, size_t value_length)
{
    ...
    while (value_index < value_length) {
        ...
        if (pattern_index < pattern_length && pattern[pattern_index] == value[value_index]) {
            pattern_index += 1u;
            value_index += 1u;
            continue;
        }
...
static size_t bc_integrity_filter_glob_count_double_star_segments(const char* pattern, size_t pattern_length)
{
    size_t count = 0u;
    ...
    while (segment_start < pattern_length) {
        ...
        if (segment_length == 2u && pattern[segment_start] == '*' && pattern[segment_start + 1u] == '*') {
            count += 1u;
        }
...
static bool bc_integrity_filter_glob_match_recursive(const char* pattern, size_t pattern_length, const char* value, size_t value_length)
{
    ...
    while (pattern_segment_start <= pattern_length) {
        ...
        while (pattern_segment_end < pattern_length && pattern[pattern_segment_end] != '/') {
            pattern_segment_end += 1u;
        }
        size_t pattern_segment_length = pattern_segment_end - pattern_segment_start;
        bool is_double_star =
            pattern_segment_length == 2u && pattern[pattern_segment_start] == '*' && pattern[pattern_segment_start + 1u] == '*';
        if (is_double_star) {
            ...
            while (true) {
                if (bc_integrity_filter_glob_match_recursive(pattern + next_pattern_start, pattern_length - next_pattern_start,
                                                             value + value_cursor, value_length - value_cursor)) {
                    return true;
                }
        ...
        while (value_segment_end < value_length && value[value_segment_end] != '/') {
            value_segment_end += 1u;
        }
        size_t value_segment_length = value_segment_end - value_segment_start;
        if (!bc_integrity_filter_glob_segment(pattern + pattern_segment_start, pattern_segment_length, value + value_segment_start,
                                              value_segment_length)) {
            return false;
        }
...
bool bc_integrity_filter_glob_matches(const char* pattern, const char* value, size_t value_length)
{
    ...
    if (bc_integrity_filter_glob_count_double_star_segments(pattern, pattern_length) > BC_INTEGRITY_FILTER_GLOB_MAX_DOUBLE_STAR_SEGMENTS) {
        return false;
    }
    return bc_integrity_filter_glob_match_recursive(pattern, pattern_length, value, value_length);
}

static bool bc_integrity_filter_pattern_directory_prefix_matches(const char* pattern, const char* value, size_t value_length)
{
    size_t pattern_length = bc_integrity_filter_cstr_length(pattern);
    if (bc_integrity_filter_glob_count_double_star_segments(pattern, pattern_length) > BC_INTEGRITY_FILTER_GLOB_MAX_DOUBLE_STAR_SEGMENTS) {
        return false;
    }
    if (pattern_length >= 3u && pattern[pattern_length - 3u] == '/' && pattern[pattern_length - 2u] == '*' &&
        pattern[pattern_length - 1u] == '*') {
        size_t prefix_length = pattern_length - 3u;
        if (bc_integrity_filter_glob_match_recursive(pattern, prefix_length, value, value_length)) {
            return true;
        }
    }
...
static bool bc_integrity_filter_count_and_split(bc_allocators_context_t* memory_context, const char* list, char** out_buffer,
                                                char*** out_patterns, size_t* out_count)
{
    ...
    char* buffer = NULL;
    if (!bc_allocators_pool_allocate(memory_context, list_length + 1u, (void**)&buffer)) {
        return false;
    }
                                ...
                                bc_integrity_filter_t** out_filter)
{
    ...
    if (!bc_allocators_pool_allocate(memory_context, sizeof(bc_integrity_filter_t), (void**)&filter)) {
        return false;
    }
    bc_core_zero(filter, sizeof(*filter));

    if (!bc_integrity_filter_count_and_split(memory_context, include_list, &filter->include_buffer, &filter->include_patterns,
                                             &filter->include_count)) {
        bc_allocators_pool_free(memory_context, filter);
        return false;
    }
    if (!bc_integrity_filter_count_and_split(memory_context, exclude_list, &filter->exclude_buffer, &filter->exclude_patterns,
...
bool bc_integrity_filter_accepts_path(const bc_integrity_filter_t* filter, const char* relative_path, size_t relative_path_length)
{
    ...
    for (size_t index = 0; index < filter->exclude_count; ++index) {
        if (bc_integrity_filter_glob_matches(filter->exclude_patterns[index], relative_path, relative_path_length)) {
            return false;
        }
    }
    ...
    for (size_t index = 0; index < filter->include_count; ++index) {
        if (bc_integrity_filter_glob_matches(filter->include_patterns[index], relative_path, relative_path_length)) {
            return true;
        }
    }
    return false;
...
bool bc_integrity_filter_accepts_directory(const bc_integrity_filter_t* filter, const char* relative_path, size_t relative_path_length)
{
    ...
    for (size_t index = 0; index < filter->exclude_count; ++index) {
        ...
        if (bc_integrity_filter_pattern_directory_prefix_matches(filter->exclude_patterns[index], relative_path, relative_path_length)) {
            return false;
        }
    }
    return true;
