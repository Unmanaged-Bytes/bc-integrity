...
static void bc_integrity_cli_spec_emit_stderr_invalid(const char* option, const char* value)
{
    char buffer[BC_INTEGRITY_CLI_STDERR_BUFFER_BYTES];
    bc_core_writer_t writer;
    if (!bc_core_writer_init_standard_error(&writer, buffer, sizeof(buffer))) {
    ...
    (void)bc_core_writer_write_cstring(&writer, "bc-integrity: invalid value for ");
...
static const char* const bc_integrity_digest_algorithm_values[] = {"sha256", "xxh3", "xxh128", NULL};

static const char* const bc_integrity_verify_mode_values[] = {"strict", "content", "meta", NULL};

static const char* const bc_integrity_output_format_values[] = {"text", "json", NULL};
...
static const bc_runtime_cli_option_spec_t bc_integrity_manifest_options[] = {
    ...
    },
    {
        ...
        .type = BC_RUNTIME_CLI_OPTION_FLAG,
        .help_summary = "include device files, fifos, and sockets (default: skip)",
    },
    {
        ...
        .help_summary = "exclude /proc /sys /dev /run /tmp from the walk (default: true)",
    },
    {
        .long_name = "include",
        .type = BC_RUNTIME_CLI_OPTION_LIST,
        .value_placeholder = "GLOB",
        .help_summary = "only include relative paths matching glob (repeatable)",
    },
    {
        .long_name = "exclude",
        .type = BC_RUNTIME_CLI_OPTION_LIST,
...
static const bc_runtime_cli_command_spec_t bc_integrity_commands[] = {
    {
        ...
        .option_count = sizeof(bc_integrity_manifest_options) / sizeof(bc_integrity_manifest_options[0]),
    ...
    },
    {
        ...
        .option_count = sizeof(bc_integrity_verify_options) / sizeof(bc_integrity_verify_options[0]),
    ...
    },
    {
        ...
        .option_count = sizeof(bc_integrity_diff_options) / sizeof(bc_integrity_diff_options[0]),
...
static const bc_runtime_cli_program_spec_t bc_integrity_program_spec_value = {
    ...
    .global_option_count = sizeof(bc_integrity_global_options) / sizeof(bc_integrity_global_options[0]),
    .commands = bc_integrity_commands,
    .command_count = sizeof(bc_integrity_commands) / sizeof(bc_integrity_commands[0]),
};
...
bool bc_integrity_cli_bind_manifest_options(const bc_runtime_config_store_t* store, const bc_runtime_cli_parsed_t* parsed,
                                            bc_integrity_manifest_options_t* out_options)
{
    ...
    if (!bc_runtime_config_store_get_string(store, "global.threads", &threads_value)) {
        bc_integrity_cli_spec_emit_stderr("bc-integrity: internal error: missing global.threads\n");
        return false;
    }
    if (!bc_integrity_cli_parse_threads(threads_value, &out_options->threads_mode, &out_options->explicit_worker_count)) {
        bc_integrity_cli_spec_emit_stderr_invalid("--threads", threads_value);
        return false;
    }

    const char* output_value = NULL;
    if (!bc_runtime_config_store_get_string(store, "manifest.output", &output_value)) {
        bc_integrity_cli_spec_emit_stderr("bc-integrity: internal error: missing manifest.output\n");
        return false;
    }
    out_options->output_path = output_value;

    const char* digest_value = NULL;
    if (!bc_runtime_config_store_get_string(store, "manifest.digest-algorithm", &digest_value)) {
        bc_integrity_cli_spec_emit_stderr("bc-integrity: internal error: missing manifest.digest-algorithm\n");
        return false;
    }
    if (!bc_integrity_cli_parse_digest_algorithm(digest_value, &out_options->digest_algorithm)) {
        bc_integrity_cli_spec_emit_stderr_invalid("--digest-algorithm", digest_value);
        return false;
    }

    bool follow_symlinks = false;
    (void)bc_runtime_config_store_get_boolean(store, "manifest.follow-symlinks", &follow_symlinks);
    out_options->follow_symlinks = follow_symlinks;

    bool include_hidden = false;
    (void)bc_runtime_config_store_get_boolean(store, "manifest.include-hidden", &include_hidden);
    out_options->include_hidden = include_hidden;

    bool include_special = false;
    (void)bc_runtime_config_store_get_boolean(store, "manifest.include-special", &include_special);
    out_options->include_special = include_special;

    bool default_exclude_virtual = true;
    (void)bc_runtime_config_store_get_boolean(store, "manifest.default-exclude-virtual", &default_exclude_virtual);
    out_options->default_exclude_virtual = default_exclude_virtual;

    const char* include_value = NULL;
    if (bc_runtime_config_store_get_string(store, "manifest.include", &include_value)) {
        out_options->include_list = include_value;
    }
    const char* exclude_value = NULL;
    if (bc_runtime_config_store_get_string(store, "manifest.exclude", &exclude_value)) {
        out_options->exclude_list = exclude_value;
    }

    if (parsed->positional_count != 1) {
        bc_integrity_cli_spec_emit_stderr("bc-integrity: manifest requires exactly one root path\n");
        return false;
    }
...
bool bc_integrity_cli_bind_verify_options(const bc_runtime_config_store_t* store, const bc_runtime_cli_parsed_t* parsed,
                                          bc_integrity_verify_options_t* out_options)
{
    ...
    if (!bc_runtime_config_store_get_string(store, "verify.mode", &mode_value)) {
        bc_integrity_cli_spec_emit_stderr("bc-integrity: internal error: missing verify.mode\n");
        return false;
    }
    ...
    if (!bc_runtime_config_store_get_string(store, "verify.format", &format_value)) {
        bc_integrity_cli_spec_emit_stderr("bc-integrity: internal error: missing verify.format\n");
        return false;
    }
    if (!bc_integrity_cli_parse_output_format(format_value, &out_options->format)) {
        bc_integrity_cli_spec_emit_stderr_invalid("--format", format_value);
        return false;
    }

    bool exit_on_first = false;
    (void)bc_runtime_config_store_get_boolean(store, "verify.exit-on-first", &exit_on_first);
    out_options->exit_on_first = exit_on_first;

    bool follow_symlinks = false;
    (void)bc_runtime_config_store_get_boolean(store, "verify.follow-symlinks", &follow_symlinks);
    out_options->follow_symlinks = follow_symlinks;

    bool include_hidden = false;
    (void)bc_runtime_config_store_get_boolean(store, "verify.include-hidden", &include_hidden);
    out_options->include_hidden = include_hidden;

    bool include_special = false;
    (void)bc_runtime_config_store_get_boolean(store, "verify.include-special", &include_special);
    out_options->include_special = include_special;

    bool default_exclude_virtual = true;
    (void)bc_runtime_config_store_get_boolean(store, "verify.default-exclude-virtual", &default_exclude_virtual);
    out_options->default_exclude_virtual = default_exclude_virtual;

    const char* include_value = NULL;
    if (bc_runtime_config_store_get_string(store, "verify.include", &include_value)) {
        out_options->include_list = include_value;
    }
    const char* exclude_value = NULL;
    if (bc_runtime_config_store_get_string(store, "verify.exclude", &exclude_value)) {
        out_options->exclude_list = exclude_value;
    }

    if (parsed->positional_count != 2) {
        bc_integrity_cli_spec_emit_stderr("bc-integrity: verify requires <root> and <manifest.hrbl>\n");
        return false;
    }
    out_options->root_path = parsed->positional_values[0];
...
bool bc_integrity_cli_bind_diff_options(const bc_runtime_config_store_t* store, const bc_runtime_cli_parsed_t* parsed,
                                        bc_integrity_diff_options_t* out_options)
{
    bc_core_zero(out_options, sizeof(*out_options));

    const char* format_value = NULL;
    if (!bc_runtime_config_store_get_string(store, "diff.format", &format_value)) {
        bc_integrity_cli_spec_emit_stderr("bc-integrity: internal error: missing diff.format\n");
        return false;
    }
    if (!bc_integrity_cli_parse_output_format(format_value, &out_options->format)) {
        bc_integrity_cli_spec_emit_stderr_invalid("--format", format_value);
    ...
    (void)bc_runtime_config_store_get_boolean(store, "diff.ignore-meta", &ignore_meta);
    out_options->ignore_meta = ignore_meta;

    bool ignore_mtime = false;
    (void)bc_runtime_config_store_get_boolean(store, "diff.ignore-mtime", &ignore_mtime);
    out_options->ignore_mtime = ignore_mtime;

    if (parsed->positional_count != 2) {
        bc_integrity_cli_spec_emit_stderr("bc-integrity: diff requires <old.hrbl> and <new.hrbl>\n");
        return false;
    }
...
const char* bc_integrity_cli_digest_algorithm_name(bc_integrity_digest_algorithm_t algorithm)
{
    switch (algorithm) {
