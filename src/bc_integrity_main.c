...
static void bc_integrity_emit_stderr_quoted_path(const char* prefix, const char* path, const char* suffix)
{
    char buffer[BC_INTEGRITY_STDERR_BUFFER_BYTES];
    bc_core_writer_t writer;
    if (!bc_core_writer_init_standard_error(&writer, buffer, sizeof(buffer))) {
...
static bool bc_integrity_command_name_equal(const char* command_name, const char* expected, size_t expected_length)
{
...
static bool bc_integrity_application_is_manifest(const bc_integrity_application_state_t* state)
{
    if (state->parsed == NULL || state->parsed->command == NULL) {
        return false;
    }
    return bc_integrity_command_name_equal(state->parsed->command->name, "manifest", 8);
}

static bool bc_integrity_application_is_verify(const bc_integrity_application_state_t* state)
{
    if (state->parsed == NULL || state->parsed->command == NULL) {
        return false;
    }
    return bc_integrity_command_name_equal(state->parsed->command->name, "verify", 6);
}

static bool bc_integrity_application_is_diff(const bc_integrity_application_state_t* state)
{
    if (state->parsed == NULL || state->parsed->command == NULL) {
        return false;
    }
    return bc_integrity_command_name_equal(state->parsed->command->name, "diff", 4);
}

static bool bc_integrity_resolve_canonical_root(const char* input_path, char* output_buffer, size_t output_capacity, size_t* out_length)
{
    char tmp[PATH_MAX];
...
static bool bc_integrity_application_init(const bc_runtime_t* application, void* user_data)
{
    bc_integrity_application_state_t* state = (bc_integrity_application_state_t*)user_data;
    ...
    if (!bc_containers_vector_create(memory_context, sizeof(bc_integrity_entry_t), BC_INTEGRITY_ENTRY_INITIAL_CAPACITY,
                                     BC_INTEGRITY_ENTRY_MAX_CAPACITY, &state->entries)) {
        state->exit_code = 1;
        return false;
    }
...
static bool bc_integrity_run_manifest(const bc_runtime_t* application, bc_integrity_application_state_t* state)
{
    bc_allocators_context_t* memory_context = NULL;
    if (!bc_runtime_memory_context(application, &memory_context)) {
        state->exit_code = 1;
    ...
    if (!bc_integrity_resolve_canonical_root(state->manifest_options.root_path, state->canonical_root_buffer,
                                             sizeof(state->canonical_root_buffer), &state->canonical_root_length)) {
        bc_integrity_emit_stderr_quoted_path("bc-integrity: cannot resolve root path '", state->manifest_options.root_path, "'\n");
        state->exit_code = 1;
        return false;
    }
    ...
    if (stat(state->canonical_root_buffer, &root_stat) != 0) {
        bc_integrity_emit_stderr_quoted_path("bc-integrity: cannot stat root '", state->canonical_root_buffer, "'\n");
        state->exit_code = 1;
        return false;
    }
    if (!S_ISDIR(root_stat.st_mode)) {
        bc_integrity_emit_stderr_quoted_path("bc-integrity: root must be a directory: '", state->canonical_root_buffer, "'\n");
        state->exit_code = 1;
        return false;
    }
    ...
    bool walk_ok = bc_integrity_walk_run(memory_context, concurrency_context, signal_handler, &state->manifest_options,
                                         state->canonical_root_buffer, state->canonical_root_length, state->entries, state->errors);
    ...
    if (state->manifest_options.defer_digest) {
        if (!bc_integrity_dispatch_compute_digests(memory_context, concurrency_context, signal_handler,
    ...
    uint64_t walltime_ms = bc_integrity_monotonic_milliseconds() - started_monotonic_ms;
    ...
    if (!bc_integrity_manifest_write_to_file(memory_context, &state->manifest_options, state->entries, &summary,
                                             state->manifest_options.output_path)) {
        bc_integrity_emit_stderr_quoted_path("bc-integrity: cannot write manifest to '", state->manifest_options.output_path, "'\n");
        state->exit_code = 1;
        return false;
    }
...
static bool bc_integrity_run_verify(const bc_runtime_t* application, bc_integrity_application_state_t* state)
{
    bc_allocators_context_t* memory_context = NULL;
    if (!bc_runtime_memory_context(application, &memory_context)) {
        state->exit_code = 1;
    ...
    int verify_exit = 2;
    if (!bc_integrity_verify_run(memory_context, concurrency_context, signal_handler, &state->verify_options, state->errors,
                                 &verify_exit)) {
        state->exit_code = 1;
        return false;
    }
...
static bool bc_integrity_run_diff(const bc_runtime_t* application, bc_integrity_application_state_t* state)
{
    bc_allocators_context_t* memory_context = NULL;
    if (!bc_runtime_memory_context(application, &memory_context)) {
        state->exit_code = 1;
    ...
    int diff_exit = 2;
    if (!bc_integrity_diff_run(memory_context, &state->diff_options, &diff_exit)) {
        state->exit_code = 1;
        return false;
    }
...
static bool bc_integrity_application_run(const bc_runtime_t* application, void* user_data)
{
    bc_integrity_application_state_t* state = (bc_integrity_application_state_t*)user_data;
    ...
    bc_integrity_emit_stderr_quoted_path("bc-integrity: ", state->parsed->command->name, ": unknown subcommand\n");
    state->exit_code = BC_INTEGRITY_EX_USAGE;
    return true;
}

static void bc_integrity_application_cleanup(const bc_runtime_t* application, void* user_data)
{
    bc_integrity_application_state_t* state = (bc_integrity_application_state_t*)user_data;
    bc_allocators_context_t* memory_context = NULL;
    if (!bc_runtime_memory_context(application, &memory_context)) {
...
int main(int argument_count, char** argument_values)
{
    ...
    if (!bc_allocators_context_create(&cli_memory_config, &cli_memory_context)) {
        bc_integrity_emit_stderr("bc-integrity: failed to initialize CLI memory context\n");
        return 1;
    }

    bc_runtime_config_store_t* cli_store = NULL;
    if (!bc_runtime_config_store_create(cli_memory_context, &cli_store)) {
        bc_integrity_emit_stderr("bc-integrity: failed to initialize CLI config store\n");
    ...
    if (bc_integrity_command_name_equal(parsed.command->name, "manifest", 8)) {
        if (!bc_integrity_cli_bind_manifest_options(cli_store, &parsed, &state.manifest_options)) {
            bc_runtime_config_store_destroy(cli_memory_context, cli_store);
            bc_allocators_context_destroy(cli_memory_context);
            return 2;
        }
    ...
    } else if (bc_integrity_command_name_equal(parsed.command->name, "verify", 6)) {
        if (!bc_integrity_cli_bind_verify_options(cli_store, &parsed, &state.verify_options)) {
            bc_runtime_config_store_destroy(cli_memory_context, cli_store);
            bc_allocators_context_destroy(cli_memory_context);
            return 2;
        }
        ...
        if (!bc_integrity_cli_bind_diff_options(cli_store, &parsed, &state.diff_options)) {
            bc_runtime_config_store_destroy(cli_memory_context, cli_store);
            bc_allocators_context_destroy(cli_memory_context);
            return 2;
        }
    ...
    if (threads_mode == BC_INTEGRITY_THREADS_MODE_MONO) {
        ...
        if (explicit_worker_count > logical_processor_count) {
            bc_integrity_emit_stderr("bc-integrity: --threads exceeds online logical processors\n");
            bc_runtime_config_store_destroy(cli_memory_context, cli_store);
            bc_allocators_context_destroy(cli_memory_context);
            return 2;
        }
